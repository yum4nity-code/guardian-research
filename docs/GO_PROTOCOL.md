...
1. Lire `docs/RESEARCH_STATUS.md`, `docs/STRATEGY_DECISIONS.md`, `CURRENT_QUEUE.json`, le dernier `PLAN.md` actif et le dernier handoff pertinent.
2. Vérifier les processus/workers MT5 réellement actifs et les tâches MiMo déjà lancées. Ne jamais créer de doublon.
3. Récolter en priorité tous les résultats MiMo/MT5 terminés depuis la dernière session, les normaliser et mettre à jour `CURRENT_QUEUE.json`.
4. Exécuter **l'action READY de priorité la plus élevée** dans la file. Ne pas demander à l'utilisateur quoi faire si la suite est déjà déterminable par l'état du labo.
5. Si la priorité principale attend encore un calcul sain, ne pas gaspiller le quota à la surveiller : passer à la prochaine action non conflictuelle utile.
6. Si aucune action READY n'existe, sélectionner la meilleure hypothèse suivante selon le mandat, créer/pré-enregistrer son plan, puis lancer le cheap-fail approprié.
7. Utiliser Codex pour l'orchestration, les décisions et le code de recherche ; déléguer les travaux longs/répétitifs à MiMo et l'exécution quantitative aux workers MT5.
8. Avant de terminer sa fenêtre, laisser au moins une charge longue utile à MiMo/MT5 si possible, puis mettre à jour `CURRENT_QUEUE.json`, `RESEARCH_STATUS.md` et `STRATEGY_DECISIONS.md` lorsque nécessaire.
9. Si quelque chose nécessite ChatGPT (candidat Guardian, anomalie produit, audit méthodologique, question d'intégration), créer/mettre à jour le handoff du jour avec les chemins exacts. Sinon, ne pas produire de handoff vide.

En résumé : **`GO` = récolter -> décider -> reprendre la meilleure action -> déléguer les calculs -> laisser une trace propre.**
...
En résumé : **`GO` à ChatGPT = ouvrir la boîte de réception GitHub Codex -> détecter le nouveau -> auditer/agir -> marquer comme traité.**
