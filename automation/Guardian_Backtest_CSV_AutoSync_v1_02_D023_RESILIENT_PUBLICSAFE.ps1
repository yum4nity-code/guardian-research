...
$TaskLabel      = 'Guardian D023 CSV AutoSync v1.02 RESILIENT PUBLICSAFE'
$InstallDir     = 'D:\MT5_Backtests\automation'
$InstalledFile  = Join-Path $InstallDir 'Guardian_Backtest_CSV_AutoSync_v1_02_D023_RESILIENT_PUBLICSAFE.ps1'
$StartupDir     = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\Startup'
$StartupCmd     = Join-Path $StartupDir 'Guardian_D023_CSV_AutoSync_v1_02.cmd'
$PidFile        = 'D:\MT5_Backtests\guardian-d023-csv-sync-v102.pid'
$StateFile      = 'D:\MT5_Backtests\guardian-d023-csv-sync-v102-state.json'
$HealthFile     = 'D:\MT5_Backtests\guardian-d023-csv-sync-v102-health.json'
$LogFile        = 'D:\MT5_Backtests\logs\guardian-d023-csv-sync-v102.log'
...
function Invoke-GitOnce([string[]]$Args, [switch]$AllowFailure) {
    $output = & git @Args 2>&1
    $code = $LASTEXITCODE
...
function Invoke-GitRetry([string[]]$Args) {
    $attempts = [Math]::Max(1,$GitAttempts)
    for ($i=1; $i -le $attempts; $i++) {
        $r = Invoke-GitOnce -Args $Args -AllowFailure
        if ($r.Code -eq 0) { return $r }
        if ($i -ge $attempts) {
            throw "git $($Args -join ' ') failed after $attempts attempts: $($r.Output -join ' ')"
        }
        $delay = [Math]::Min(60,[Math]::Pow(2,$i))
        Write-Log "WARN git retry $i/$attempts in ${delay}s | $($Args -join ' ')"
        Start-Sleep -Seconds ([int]$delay)
    }
}

function Get-RemoteUrl {
    foreach ($candidate in @('D:\MT5_Backtests\guardian-research',(Split-Path -Parent $PSScriptRoot))) {
        if ($candidate -and (Test-Path -LiteralPath (Join-Path $candidate '.git'))) {
            try {
                $r = Invoke-GitOnce -Args @('-C',$candidate,'remote','get-url','origin')
...
function Commit-PendingIfAny {
    $status = Invoke-GitOnce -Args @('-C',$ResultsClone,'status','--porcelain','--','backtests/inbox')
    if ($status.Output.Count -eq 0) { return }
    Invoke-GitOnce -Args @('-C',$ResultsClone,'add','--','backtests/inbox') | Out-Null
    $c = Invoke-GitOnce -Args @('-C',$ResultsClone,'-c','user.name=Guardian Backtest Bot','-c','user.email=guardian-backtest@local','commit','-m','Recover pending D023 AutoSync transaction') -AllowFailure
    if ($c.Code -ne 0) {
        $after = Invoke-GitOnce -Args @('-C',$ResultsClone,'status','--porcelain','--','backtests/inbox')
        if ($after.Output.Count -gt 0) { throw "cannot recover pending AutoSync transaction: $($c.Output -join ' ')" }
    }
}
...
function Ensure-ResultsClone {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $ResultsClone '.git'))) {
        ...
        Invoke-GitRetry -Args @('clone','--quiet','--branch',$ResultsBranch,'--single-branch',$remote,$ResultsClone) | Out-Null
    }

    $branch = Invoke-GitOnce -Args @('-C',$ResultsClone,'rev-parse','--abbrev-ref','HEAD')
    if ((($branch.Output | Select-Object -First 1).ToString()).Trim() -ne $ResultsBranch) {
        Invoke-GitRetry -Args @('-C',$ResultsClone,'checkout','-q',$ResultsBranch) | Out-Null
    }

    Commit-PendingIfAny
    Invoke-GitRetry -Args @('-C',$ResultsClone,'pull','--rebase','origin',$ResultsBranch) | Out-Null
    Invoke-GitRetry -Args @('-C',$ResultsClone,'push','origin',$ResultsBranch) | Out-Null
}
...
function Publish-D023([string]$StatsPath, [string]$TradesPath) {
    ...
    $existingState = Get-StateFingerprint
    if ($existingState -eq $fingerprint) {
    ...
    if (Test-Path -LiteralPath $manifestPath) {
        try {
            ...
            if ($old.fingerprint -eq $fingerprint) {
                ...
                Write-Health -Status 'OK' -Message 'D023 publication already present on local synced branch.' -Fingerprint $fingerprint
                return $false
            }
        } catch {}
    ...
    $publishedStats = Import-D023 -StatsPath $statsDest -TradesPath $tradesDest
    if ($publishedStats.TradesClosed -ne $validated.TradesClosed) { throw 'published D023 snapshot validation mismatch.' }
    ...
    $manifest = [ordered]@{
        ...
        validation = [ordered]@{
            ...
            symbol = $publishedStats.Final.symbol
            timeframe = $publishedStats.Final.timeframe
            trades_closed = $publishedStats.TradesClosed
            csv_trade_rows = [int]$publishedStats.Final.csv_trade_rows
            published_trades_rows = $publishedStats.Trades.Count
        }
        files = @(
    ...
    Invoke-GitOnce -Args @('-C',$ResultsClone,'add','--',$rel,'backtests/inbox/LATEST.json') | Out-Null
    $commit = Invoke-GitOnce -Args @('-C',$ResultsClone,'-c','user.name=Guardian Backtest Bot','-c','user.email=guardian-backtest@local','commit','-m',"Auto-sync validated D023 CSV $runId") -AllowFailure
    if ($commit.Code -ne 0) {
        $status = Invoke-GitOnce -Args @('-C',$ResultsClone,'status','--porcelain','--','backtests/inbox')
...
function Install-Agent {
    ...
    $cmd = '@echo off' + "`r`n" + 'start "Guardian D023 CSV Sync" /min powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "' + $InstalledFile + '"' + "`r`n"
...
function Uninstall-Agent {
    ...
    Write-Host 'Guardian D023 CSV AutoSync v1.02 desactive.'
}
...
$createdNew = $false
$mutex = New-Object System.Threading.Mutex($true,'Local\GuardianD023CsvAutoSyncV102ResilientPublicSafe',[ref]$createdNew)
...
try {
    ...
    Write-Health -Status 'STARTING' -Message 'Waiting for finalized D023 v1.08 outputs.' -Fingerprint ''
    while ($true) {
        try {
            if (-not (Test-Path -LiteralPath $statsPath) -or -not (Test-Path -LiteralPath $tradesPath)) {
                Write-Health -Status 'WAITING_OUTPUTS' -Message 'D023 STATS/TRADES pair not present yet.' -Fingerprint ''
            } else {
                ...
                if ($Once -or $stableFor -ge $StableSeconds) {
                    ...
                    if ($rawTail -match '(^|;)FINAL(;|$)') {
                        ...
                        Write-Health -Status 'WAITING_FINAL' -Message 'D023 files exist but STATS FINAL is not present yet.' -Fingerprint ''
                    }
                }
            }
        } catch {
            ...
            Write-Health -Status 'ERROR_RETRYING' -Message $msg -Fingerprint ''
        }
