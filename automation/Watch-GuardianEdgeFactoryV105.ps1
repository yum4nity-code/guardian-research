...
$Base=Join-Path $Root "Research\Autonomous\guardian_edge_factory_v105_cftc_preoos_forensic"
if(!(Test-Path $Base)){ throw "No V105 base directory yet: $Base" }
$Run=Get-ChildItem $Base -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if(!$Run){ throw "No V105 run directory yet" }
Write-Host "=== V105 LATEST RUN ==="
