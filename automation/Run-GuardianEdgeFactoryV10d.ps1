...
OUTB=B/"guardian_edge_factory_v10d";OUTB.mkdir(parents=True,exist_ok=True);t0=time.time()
def prog(o,s,i,n,m=""):
...
# Search local source code for cache5/resample construction; pre-OOS data only.
hits=[]
roots=[REPO,B/"guardian_edge_factory_v3"]
files=[]
for rr in roots:
 if rr.exists():
  files += [p for p in rr.rglob("*") if p.is_file() and p.suffix.lower() in [".py",".ps1",".md",".json"] and "GEF10D-" not in str(p)]
for i,p in enumerate(files,1):
 try:
  txt=p.read_text(errors="ignore")
  for ln,line in enumerate(txt.splitlines(),1):
   if re.search(r"cache5|resample\(|5min|5T|floor\(|ceil\(",line,re.I):
    hits.append({"file":str(p),"line":ln,"text":line[:1000]})
 except: pass
 if i%100==0: prog(OUT,"CODE TRACE",i,len(files),p.name)
pd.DataFrame(hits).to_csv(OUT/"cache5_code_trace.csv",index=False)
# Load raw only for six known events and reconstruct plausible M5 conventions.
...
# Candidate semantics. Standard pandas combinations + explicit windows around label t.
cands=[]
for t in events:
 d=raws[t.year]
 cc=float(cache.loc[t,"close"])
 rec={"time":t,"cache_close":cc}
 # Explicit possible close sources t+k, k -5..+5
 for k in range(-5,6):
  tt=t+pd.Timedelta(minutes=k)
  rec[f"raw_t{k:+d}"]=float(d.loc[tt,"close"]) if tt in d.index else np.nan
 # Standard resamples on local ±1d data
 ...
 for label in ["left","right"]:
  for closed in ["left","right"]:
   ...
   key=f"{label}_{closed}"
   rec[key]=float(rs.loc[t,"close"]) if t in rs.index else np.nan
 cands.append(rec)
C=pd.DataFrame(cands)
# determine exact matching semantics
candidate_cols=[c for c in C.columns if c not in ["time","cache_close"]]
scores=[]
for c in candidate_cols:
 ok=np.isclose(C[c],C.cache_close,rtol=0,atol=1e-12,equal_nan=False)
 scores.append({"candidate":c,"matches":int(ok.sum()),"n":len(C),"fraction":float(ok.mean()),"causal_at_label": (c in ["raw_t+0","right_right","right_left"] or c.startswith("raw_t-"))})
S=pd.DataFrame(scores).sort_values(["matches","candidate"],ascending=[False,True]);S.to_csv(OUT/"candidate_semantics_scores.csv",index=False);C.to_csv(OUT/"event_semantics_matrix.csv",index=False)
best=S.iloc[0].to_dict()
# Also test all cache timestamps in event windows against likely future close offsets.
offset_stats=[]
for k in range(-5,6):
 n=match=0
 for t0 in events:
  d=raws[t0.year]; times=cache.loc[(cache.index>=t0-pd.Timedelta(minutes=60))&(cache.index<=t0+pd.Timedelta(minutes=60))].index
  for t in times:
   tt=t+pd.Timedelta(minutes=k)
   if tt in d.index:
    n+=1; match+=int(abs(float(cache.loc[t,"close"])-float(d.loc[tt,"close"]))<1e-12)
 offset_stats.append({"offset_min":k,"matches":match,"n":n,"fraction":match/n if n else np.nan,"available_by_cache_label":k<=0})
O=pd.DataFrame(offset_stats).sort_values("fraction",ascending=False);O.to_csv(OUT/"offset_match_scores.csv",index=False)
# Verdict: future offset exact match across overwhelming majority is direct lookahead evidence.
bo=O.iloc[0]
...
print("\n=== V10D RECEIPT ===");print(json.dumps(receipt,indent=2,default=str))
print("\n=== OFFSET MATCH SCORES ===");print(O.to_string(index=False))
print("\n=== EVENT SEMANTICS ===");print(C.to_string(index=False))
print("\n=== CODE TRACE (first 40) ===");print(pd.DataFrame(hits).head(40).to_string(index=False))
print("\nRUN:",OUT)
'@
