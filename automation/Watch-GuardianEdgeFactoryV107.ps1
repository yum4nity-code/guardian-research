...
$Base=Join-Path $Root "Research\Autonomous\guardian_edge_factory_v107_alfred_event_level"
if(!(Test-Path $Base)){ throw "No V107 base directory yet: $Base" }
$Run=Get-ChildItem $Base -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if(!$Run){ throw "No V107 run directory yet" }
Write-Host "=== V107 LATEST RUN ==="
