...
$TaskLabel      = 'Guardian Backtest CSV AutoSync v1'
$InstallDir     = 'D:\MT5_Backtests\automation'
$InstalledFile  = Join-Path $InstallDir 'Guardian_Backtest_CSV_AutoSync_v1.ps1'
        ...
        try {
            Get-ChildItem -LiteralPath $root -Filter '*.csv' -File -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                if (-not $_.FullName.StartsWith($ResultsClone,[System.StringComparison]::OrdinalIgnoreCase) -and
                    $_.Length -gt 0 -and $_.Length -le $MaxBytes) {
...
function Install-Agent {
    ...
    Write-Host "GITHUB: branche $ResultsBranch / backtests/inbox/"
}
...
$createdNew = $false
$mutex = New-Object System.Threading.Mutex($true,'Local\GuardianBacktestCsvAutoSyncV1',[ref]$createdNew)
...
try {
    Write-Log "$TaskLabel START | pid=$PID | stable=${StableSeconds}s | recent=${RecentMinutes}m"
    while ($true) {
        $now = [DateTime]::UtcNow
