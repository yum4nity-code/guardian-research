...
$Base=Join-Path $Root "Research\Autonomous\guardian_edge_factory_v109_treasury_event_level"
if(!(Test-Path $Base)){ throw "No V109 base directory yet: $Base" }
$Run=Get-ChildItem $Base -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if(!$Run){ throw "No V109 run directory yet" }
Write-Host "=== V109 LATEST RUN ==="
