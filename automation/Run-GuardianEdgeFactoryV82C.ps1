...
rid="GEF82C-"+pd.Timestamp.now("UTC").strftime("%Y%m%d-%H%M%S")
O=ROOT/"Research"/"Autonomous"/"guardian_edge_factory_v82c"/rid
...
def prog(i,n,msg):
    ...
    print(f"[GEF82C] {i}/{n} {100*i/n:.0f}% | elapsed {e:.1f}s | ETA {eta:.1f}s | {msg}",flush=True)
...
for j,p in enumerate(use,1):
    with zipfile.ZipFile(p) as z:
        members=[x for x in z.namelist() if x.lower().endswith((".xls",".xlsx"))]
        for m in members:
            ...
            for c in datecols[:8]:
                ...
                samples.append({
                    ...
                    "sample_values":[str(x) for x in s.dropna().head(12).tolist()]
                })
            ...
            rows.append({
                ...
                "member":m,
                "rows":len(d),
                "date_columns":"|".join(map(str,datecols)),
                "market_columns":"|".join(map(str,marketcols))
            })

    if j==1 or j==len(use) or j%2==0:
        prog(2,6,f"inspected {j}/{len(use)} archives")

pd.DataFrame(rows).to_csv(O/"TABLES.csv",index=False)
(O/"DATE_SAMPLES.json").write_text(json.dumps(samples,indent=2),encoding="utf-8")
...
for x in samples:
    ...
    if vals:
        ...
        six=sum(bool(re.fullmatch(r"\d{6}",v)) for v in compact)
        eight=sum(bool(re.fullmatch(r"\d{8}",v)) for v in compact)
        threshold=max(1,len(compact)//2)
        if six>=threshold:
            rule="YYMMDD_EXPLICIT_FORMAT_%y%m%d"
        elif eight>=threshold:
            rule="YYYYMMDD_EXPLICIT_FORMAT_%Y%m%d"
        else:
            rule="GENERIC_DATETIME"
...
receipt={
    "run_id":rid,
    "status":"COMPLETE_CFTC_DATE_FORENSIC",
    "archives":len(use),
    "date_fields":len(samples),
    "parser_rule_counts":counts,
    "edge_trials":0,
    "2014_plus_accessed":False,
    "2023_plus_accessed":False,
    "protected_2026_accessed":False,
    "next":"repair V82B date parser only after inspecting this receipt"
}
(O/"RUN_RECEIPT.json").write_text(json.dumps(receipt,indent=2),encoding="utf-8")
prog(5,6,f"rules={counts}")
prog(6,6,"STOP")

print("\n=== V82C RECEIPT ===")
print(json.dumps(receipt,indent=2))
print("\n=== DATE PARSER RECOMMENDATIONS ===")
if rules:
    print(pd.DataFrame(rules)[["archive","column","dtype","sample_values","recommended_parser"]].head(30).to_string(index=False))
else:
    print("NONE")
print("\nRUN:",O)
'@
