...
def recurring_tradability(close):
    idx=close.index
    disc=(idx>=DISCOVERY_START)&(idx<DISCOVERY_END)
    present=close.notna().to_numpy()
    slots=slot_of_week(idx)
    ratios=np.full(SLOTS_PER_WEEK,np.nan,dtype=float)
    for s in range(SLOTS_PER_WEEK):
        m=disc&(slots==s)
        if m.any(): ratios[s]=present[m].mean()
    recurring=np.isfinite(ratios)&(ratios>=TRADABILITY_MIN)
    base=recurring.copy()
    buffered=base.copy()
    # Circular transition boundaries. Invalidate 30m on the valid side of every close/reopen.
    for s in range(SLOTS_PER_WEEK):
        prev=base[(s-1)%SLOTS_PER_WEEK];cur=base[s]
        if prev and not cur: # close boundary: remove preceding valid slots
            for k in range(1,BUFFER_SLOTS+1):
                buffered[(s-k)%SLOTS_PER_WEEK]=False
        if (not prev) and cur: # reopen boundary: remove following valid slots
            for k in range(0,BUFFER_SLOTS):
                buffered[(s+k)%SLOTS_PER_WEEK]=False
    mask=np.array([bool(buffered[int(s)]) for s in slots],dtype=bool)&present
    return pd.Series(mask,index=idx,dtype=bool),ratios,base,buffered
...
def main():
    ...
    spec=repo/"research"/"campaigns"/"GUARDIAN_M5_MOTION_TOPOLOGY_FACTORY_V1_2026_09_22.json"
    ...
    if frozen.get("status")!="DESIGN_FROZEN_BEFORE_COMPUTE": raise RuntimeError("M5 topology design not frozen")
    ...
    for i,sym in enumerate(MARKETS,1):
        ...
        trad,ratios,base_slots,buffered=recurring_tradability(bars["close"])
        ...
        coverage.append({
          ...
          "tradable_rows_discovery":int(trad.loc[disc].sum()),"recurring_slots_pre_buffer":int(base_slots.sum()),
          "recurring_slots_post_buffer":int(buffered.sum()),"m1_rows_by_year_json":json.dumps(meta["m1_rows_by_year"],sort_keys=True)
        })
        for slot in range(SLOTS_PER_WEEK):
            ratio_rows.append({"market":sym,"slot_of_week":slot,"availability_ratio":ratios[slot],
                               "recurring_pre_buffer":bool(base_slots[slot]),"recurring_post_buffer":bool(buffered[slot])})
    ...
    pd.DataFrame(ratio_rows).to_csv(out/"TRADABILITY_SLOT_AUDIT.csv",index=False)
    cov=pd.DataFrame(coverage);cov.to_csv(out/"MARKET_COVERAGE.csv",index=False)
    status(3,10,"recurring tradability masks frozen",mask_rows=len(TM))
    ...
    manifest={
      ...
      "tradability_rule":{"availability_ratio_min":TRADABILITY_MIN,"buffer_minutes":BUFFER_SLOTS*5,
                           "source_years_for_slot_ratio":[2012,2013,2014]},
      "endpoint_rule":{
    ...
    receipt={
      "run_id":rid,"status":"COMPLETE_M5_MOTION_TOPOLOGY_CACHE_BUILD",
      ...
      "next":"AUDIT CACHE AND PREREGISTER M01-M08 DISCOVERY TESTS BEFORE ANY OUTCOME ASSOCIATION"
    }
