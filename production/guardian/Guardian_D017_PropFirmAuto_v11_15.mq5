...
input bool              InpEnableCryptoDebugLogs = false; // Journaux détaillés des filtres crypto
...
input bool              InpAutoAddTPToManual      = false;  // v11.15: legacy, pas de TP broker intégral; TP partiel logiciel + runner
...
input int               InpBreakoutExitDonchian    = 10;
...
input bool              InpAllowBreakout           = false;
input bool              InpAllowPullback           = false;
input bool              InpAllowSweep              = false;
input bool              InpAllowMomentum           = true;

input group "=== [5D] CRYPTO SWEEP / RECLAIM ==="
input bool              InpCryptoSweepEnabled      = true;
input ENUM_TIMEFRAMES   InpCryptoSweepTF           = PERIOD_M5;
input int               InpCryptoSweepLookback     = 24;
input double            InpCryptoSweepMinATR       = 1.20;
input double            InpCryptoSweepMaxATR       = 4.50;
input double            InpCryptoSweepMinWickPct   = 25.0;
input double            InpCryptoSweepReclaimPct   = 55.0;
input int               InpCryptoSweepConfirmBars  = 3;
input double            InpCryptoSweepSL_ATR       = 0.35;
input double            InpCryptoSweepTP_R         = 2.00;
input double            InpCryptoSweepRiskPct      = 0.25;

input group "=== [5] MOTEURS STRATÉGIQUES & FILTRES PRO ==="
input ENUM_STRATEGY_MODE InpStrategyMode          = MODE_PORTFOLIO_RANKED;
...
input int               InpPullbackLookbackBars    = 6;      // Barres historiques utilisées pour identifier l'impulsion
input double            InpPullbackMinImpulseATR   = 0.80;   // Taille minimale de l'impulsion en ATR
input double            InpPullbackMinRetracePct   = 20.0;   // Retracement minimal de l'impulsion
input double            InpPullbackMaxRetracePct   = 70.0;   // Retracement maximal : évite les retournements déguisés
input double            InpPullbackMinConfirmBodyATR= 0.15;  // Corps minimal de la bougie de reprise
...
input double            InpClassicSessionStartUTC = 7.0;   // D-015 : ouverture Europe liquide
input double            InpClassicSessionEndUTC   = 17.0;  // D-015 : fin au déclin de liquidité Londres
...
input double            InpBreakoutBE_R             = 1.50;
input double            InpBreakoutTrailATR         = 2.50;
input double            InpPullbackTP1_R            = 2.00;
input double            InpPullbackTP1_ClosePct     = 40.0;
input double            InpPullbackBE_R             = 1.25;
input double            InpPullbackTrailATR         = 2.00;
input double            InpSweepTP1_R               = 1.00;
input double            InpSweepTP1_ClosePct        = 50.0;
input double            InpSweepBE_R                 = 0.75;
input double            InpSweepTrailATR             = 1.00;
...
input int               InpBreakoutMaxMinutes        = 240;
input int               InpPullbackMaxMinutes        = 45;
input int               InpSweepMaxMinutes           = 30;
input int               InpMomentumMaxMinutes        = 60;
input double            InpBreakoutMinProgressR      = 0.30;
input double            InpPullbackMinProgressR      = 0.30;
input double            InpSweepMinProgressR         = 0.50;
input double            InpMomentumMinProgressR      = 0.40;
input bool              InpEnableStrategyTimeStop   = true;

input group "=== [9] BLACKBOX & LEDGER (AUCUNE MESSAGERIE EXTERNE) ==="
input bool              InpEnableBlackBox         = true;   // CSV dans le dossier commun MT5/Files
input bool              InpEnableUnifiedLedger    = true;   // Journal analytique unique par compte
...
int OnInit()
  {
   ...
   PrintFormat("==================================================");
   PrintFormat(" Guardian D017 / PropFirmAuto v11.15");
...
void UpdateHUD()
  {
   ...
   string hud = "==================================================\n";
   hud += "   GUARDIAN D017 / PROP FIRM AUTO v11.15       \n";
   hud += "==================================================\n";
