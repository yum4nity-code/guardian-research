...
// PROD RISKFIX: plancher minimum auto-trade self-contained = 0,25 % du capital de reference.
// 10k -> 25 USD | 100k -> 250 USD. Un signal scale sous ce plancher est bloque, jamais remonte.
...
input bool              InpAutoAddTPToManual      = false;  // v11.16.1: legacy, pas de TP broker intégral; TP partiel logiciel + runner
...
bool RiskFloorSelfTest()
  {
   ...
   const double v100k=MinTradeRiskUSDForCapital(100000.0);
   const bool ok=(MathAbs(v10k-25.0)<=0.000001 && MathAbs(v100k-250.0)<=0.000001);
   if(ok)
      PrintFormat("RISK_FLOOR_SELFTEST PASS | 10k=%.2f | 100k=%.2f | current=%.2f",v10k,v100k,EffectiveMinTradeRiskUSD());
   else
      PrintFormat("RISK_FLOOR_SELFTEST FAIL | 10k=%.8f | 100k=%.8f",v10k,v100k);
   return ok;
  }
...
bool StrategyAllowedForProfile(ENUM_AUTO_STRATEGY st,ENUM_ASSET_PROFILE p,ENUM_MARKET_REGIME r)
  {
   // PROD D017 v11.16.1 RISKFIX : Momentum uniquement. Breakout/Pullback/Sweep restent dans le source
...
int OnInit()
  {
   ...
   if(g_detected_base_cap<=0.0 || !RiskFloorSelfTest())
     {
      Print("[ERREUR FATALE] RISK_FLOOR_SELFTEST_FAIL | capital/plancher invalide, auto-trading bloque.");
      return INIT_FAILED;
     }
   ...
   PrintFormat("==================================================");
   PrintFormat(" Guardian D017 / PropFirmAuto v11.16.1 RISKFIX MOMENTUM PROD");
...
void UpdateHUD()
  {
   ...
   string hud = "==================================================\n";
   hud += "   GUARDIAN D017 / PROP FIRM AUTO v11.16.1 RISKFIX       \n";
