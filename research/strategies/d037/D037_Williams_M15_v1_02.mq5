...
// research/campaigns/D037_EXECUTION_CLARIFICATION_ZERO_RANGE_REFERENCE_DAY_2026_09_06.md
//
// Frozen strategy:
...
// - if previous completed D1 prices are valid but High(D-1)==Low(D-1), the current
//   broker day has no positive D037 volatility reference and is skipped without a trade.
...
string SOURCE_NAME="D037_Williams_M15_v1_02.mq5";
...
bool g_day_no_reference_range=false;
...
long g_no_reference_range_days=0;
long g_signals=0;
...
void SetFatal(string status,string message)
{
   ...
   PrintFormat("D037 V102 FATAL | %s | %s",status,message);
}
...
bool CalcMoney1Lot(double entry_px,double exit_px,bool long_side,double &pnl,bool for_risk)
{
   ...
   if((StringFind(_Symbol,"EURUSD")>=0 || StringFind(_Symbol,"GBPUSD")>=0) && AssetClass()=="FOREX")
   {
      ...
      if(contract>0.0 && MathIsValidNumber(contract))
      {
         ...
         if(MathIsValidNumber(pnl))
         {
            ...
            PrintFormat("D037 V102 %s FALLBACK | symbol=%s | primary_err=%d | entry=%.10f | exit=%.10f | pnl=%.8f",
   ...
   PrintFormat("D037 V102 %s CALC FAILED | symbol=%s | primary_err=%d | entry=%.10f | exit=%.10f",
...
void ResetDay()
{
   ...
   g_day_no_reference_range=false;
}
...
void LogInvalidDaySetup(const MqlRates &cur,const MqlRates &prev,double prev_range,double long_trigger,double short_trigger)
{
   PrintFormat("D037 V102 INVALID_DAY_SETUP | symbol=%s | cur_time=%s | cur_open=%.10f | prev_time=%s | prev_open=%.10f | prev_high=%.10f | prev_low=%.10f | prev_close=%.10f | prev_range=%.10f | long_trigger=%.10f | short_trigger=%.10f",
               _Symbol,TimeToString(cur.time,TIME_DATE|TIME_MINUTES),cur.open,
               TimeToString(prev.time,TIME_DATE|TIME_MINUTES),prev.open,prev.high,prev.low,prev.close,
               prev_range,long_trigger,short_trigger);
}
...
bool LoadDaySetup(datetime t)
{
   ...
   {
      LogInvalidDaySetup(cur,prev,prev_range,0.0,0.0);
      g_invalid_price++;
   ...
   if(prev_range==0.0)
   {
      g_long_trigger=0.0;
      g_short_trigger=0.0;
      g_day_no_reference_range=true;
      g_no_reference_range_days++;
      PrintFormat("D037 V102 SKIP_NO_REFERENCE_RANGE | symbol=%s | day=%s | previous_d1=%s | prev_high=%.10f | prev_low=%.10f",
                  _Symbol,TimeToString(cur.time,TIME_DATE),TimeToString(prev.time,TIME_DATE),prev_high,prev_low);
      return true;
   }

   double long_trigger=day_open+0.50*prev_range;
   double short_trigger=day_open-0.50*prev_range;
   if(!ValidPrice(long_trigger) || !ValidPrice(short_trigger))
   {
      LogInvalidDaySetup(cur,prev,prev_range,long_trigger,short_trigger);
      g_invalid_price++;
      SetFatal("FINAL_INVALID_PRICE","invalid broker-day derived trigger");
      return false;
   }
...
void WriteStats(string status)
{
   ...
   FileWrite(g_stats_fh,status,SOURCE_NAME,SOURCE_VERSION,StageName(),
      g_bars_seen,g_bars_in_stage,g_days_initialized,g_days_traded,g_ambiguous_days,g_no_reference_range_days,g_signals,
...
{
   if(g_in_trade || g_day_traded || g_day_ambiguous || g_day_no_reference_range || !InStage(g_day_start)) return;
...
int OnInit()
{
   ...
   {
      PrintFormat("D037 V102 FATAL wrong timeframe | got=%s | required=PERIOD_M15",EnumToString(_Period));
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!InpWriteCSV)
   {
      Print("D037 V102 FATAL InpWriteCSV must remain true.");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!IsAllowedSymbol())
   {
      PrintFormat("D037 V102 FATAL symbol outside frozen universe | %s",_Symbol);
      return INIT_PARAMETERS_INCORRECT;
   }
   string account_ccy=AccountInfoString(ACCOUNT_CURRENCY);
   if(account_ccy!="USD")
   {
      PrintFormat("D037 V102 FATAL account currency must be USD | got=%s",account_ccy);
      return INIT_PARAMETERS_INCORRECT;
   }

   string sym=CleanSymbol();
   string stage=StageName();
   g_stats_name=StringFormat("D037_V102_%s_%s_STATS.csv",stage,sym);
   g_trades_name=StringFormat("D037_V102_%s_%s_TRADES.csv",stage,sym);
   ...
   {
      PrintFormat("D037 V102 FATAL cannot open STATS | err=%d | %s%s",GetLastError(),g_common_path,g_stats_name);
      return INIT_FAILED;
   }
   FileWrite(g_stats_fh,"status","source_name","source_version","run_stage","bars_seen","bars_in_stage",
      "days_initialized","days_traded","ambiguous_days","no_reference_range_days","entry_signals","trades_opened","trades_closed",
   ...
   if(g_trades_fh==INVALID_HANDLE)
   {
      ...
      PrintFormat("D037 V102 FATAL cannot open TRADES | err=%d | %s%s",GetLastError(),g_common_path,g_trades_name);
   ...
   PrintFormat("D037 V102 READY | source=%s | stage=%s | symbol=%s | M15 | class=%s | NO ORDERS",SOURCE_NAME,stage,_Symbol,AssetClass());
...
void OnDeinit(const int reason)
{
   ...
   PrintFormat("D037 V102 FINAL | status=%s | stage=%s | symbol=%s | opened=%I64d | closed=%I64d | rows=%I64d | ambiguous_days=%I64d | no_reference_range_days=%I64d | invalid_price=%I64d | invalid_risk=%I64d | risk_fallbacks=%I64d | pnl_fallbacks=%I64d | pnl_failures=%I64d",
      final_status,StageName(),_Symbol,g_trades_opened,g_trades_closed,g_csv_rows,g_ambiguous_days,g_no_reference_range_days,g_invalid_price,g_invalid_risk,g_risk_fallbacks,g_pnl_fallbacks,g_pnl_calc_failures);
}
