...
string SOURCE_NAME="D037_WilliamsPrevDayRangeBreakout_M15_v1_00_FUNDEDNEXT_DIRECT_20260906.mq5";
...
void SetFatal(string status,string message)
{
   ...
   PrintFormat("D037 V100 FATAL | %s | %s",status,message);
}
...
bool CalcMoney1Lot(double entry_px,double exit_px,bool long_side,double &pnl,bool for_risk)
{
   ...
   // Exact USD-quoted FX fallback, only after both market prices passed validation.
   if((StringFind(_Symbol,"EURUSD")>=0 || StringFind(_Symbol,"GBPUSD")>=0) && AssetClass()=="FOREX")
   {
      ...
      if(contract>0.0 && MathIsValidNumber(contract))
      {
         ...
         if(MathIsValidNumber(pnl))
         {
            ...
            PrintFormat("D037 V100 %s FALLBACK | symbol=%s | primary_err=%d | entry=%.10f | exit=%.10f | pnl=%.8f",
   ...
   PrintFormat("D037 V100 %s CALC FAILED | symbol=%s | primary_err=%d | entry=%.10f | exit=%.10f",
...
void OpenFromBar(const MqlRates &b,bool long_side,double entry_px)
{
   ...
   // Conservative same-bar sequencing: if the entry bar also contains the stop,
   // assume the stop was hit after entry.
   EvaluateStopBar(b);
}
...
int OnInit()
{
   ...
   {
      PrintFormat("D037 V100 FATAL wrong timeframe | got=%s | required=PERIOD_M15",EnumToString(_Period));
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!InpWriteCSV)
   {
      Print("D037 V100 FATAL InpWriteCSV must remain true.");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!IsAllowedSymbol())
   {
      PrintFormat("D037 V100 FATAL symbol outside frozen universe | %s",_Symbol);
      return INIT_PARAMETERS_INCORRECT;
   }
   string account_ccy=AccountInfoString(ACCOUNT_CURRENCY);
   if(account_ccy!="USD")
   {
      PrintFormat("D037 V100 FATAL account currency must be USD | got=%s",account_ccy);
      return INIT_PARAMETERS_INCORRECT;
   }

   string sym=CleanSymbol();
   string stage=StageName();
   g_stats_name=StringFormat("D037_V100_%s_%s_STATS.csv",stage,sym);
   g_trades_name=StringFormat("D037_V100_%s_%s_TRADES.csv",stage,sym);
   ...
   {
      PrintFormat("D037 V100 FATAL cannot open STATS | err=%d | %s%s",GetLastError(),g_common_path,g_stats_name);
      return INIT_FAILED;
   }
   ...
   if(g_trades_fh==INVALID_HANDLE)
   {
      ...
      PrintFormat("D037 V100 FATAL cannot open TRADES | err=%d | %s%s",GetLastError(),g_common_path,g_trades_name);
   ...
   PrintFormat("D037 V100 READY | source=%s | stage=%s | symbol=%s | M15 | class=%s | NO ORDERS",SOURCE_NAME,stage,_Symbol,AssetClass());
...
void OnDeinit(const int reason)
{
   ...
   PrintFormat("D037 V100 FINAL | status=%s | stage=%s | symbol=%s | opened=%I64d | closed=%I64d | rows=%I64d | ambiguous_days=%I64d | invalid_price=%I64d | invalid_risk=%I64d | risk_fallbacks=%I64d | pnl_fallbacks=%I64d | pnl_failures=%I64d",
