...
#property description "D038 NR7 volatility contraction breakout V0 - tick execution and native Trade Path telemetry"

// D038 V0 frozen research harness.
// NO ORDERS. Simulates entries/exits from executable tester ticks.
// Preregistration:
// research/campaigns/D038_NR7_VOLATILITY_CONTRACTION_BREAKOUT_V0_PREREGISTRATION_2026_09_06.md
// Trade Path contract:
// research/runner/TRADE_PATH_DATASET_SPEC.md
//
// Frozen strategy:
// - previous completed broker D1 bar must be strict NR7 (smallest positive range of D-1..D-7)
...
// - max one trade per setup/day, no reversal
// - no TP, trailing, BE, partial, indicator or regime filter
// - exit at stop, last tick before broker-day change, or tester end
// - baseline FundedNext commission + x1.5 commission stress
// - native executable-side MFE/MAE and R milestone telemetry
//
// Stage selection is intentionally NOT encoded in the EA. Tester FromDate/ToDate
// selects smoke/development/confirmation; output run_stage token remains RUN.

input bool InpWriteCSV=true;

string EXPERIMENT_ID="D038-NR7-VOLATILITY-CONTRACTION-BREAKOUT-V0";
string SOURCE_NAME="D038_NR7_TickPath_M15_v1_00.mq5";
...
double g_nr7_high=0.0;
double g_nr7_low=0.0;
double g_nr7_range=0.0;
...
// Last executable tick for EOD/test-end closing
bool g_have_last_tick=false;
MqlTick g_last_tick;

// Native Trade Path state
double g_mfe_r=0.0;
double g_mae_r=0.0;
...
long g_nr7_setup_days=0;
long g_non_nr7_days=0;
...
void SetFatal(string status,string message)
{
   ...
   PrintFormat("D038 V100 FATAL | %s | %s",status,message);
}
...
bool CalcMoney1LotCore(double entry_px,double exit_px,bool long_side,double &pnl,bool &used_fallback,int &primary_err)
{
   ...
   // USD-quoted forex fallback only. USDJPY requires broker conversion and must use OrderCalcProfit.
...
bool RiskMoney(double entry_px,double stop_px,bool long_side,double &risk_money)
{
   ...
   {
      PrintFormat("D038 V100 RISK CALC FAILED | symbol=%s | err=%d | entry=%.10f | stop=%.10f",
...
bool PnlMoney(double entry_px,double exit_px,bool long_side,double &pnl)
{
   ...
   {
      PrintFormat("D038 V100 PNL CALC FAILED | symbol=%s | err=%d | entry=%.10f | exit=%.10f",
...
bool PathR(double exit_px,double &r)
{
   ...
   {
      PrintFormat("D038 V100 PATH CALC FAILED | symbol=%s | err=%d | entry=%.10f | exit=%.10f",
...
void ObservePath(const MqlTick &tick)
{
   ...
   // Post-milestone min/max after first +1R, +2R, +3R.
   for(int j=0;j<3;j++)
   {
      int level_index=j+1;
...
bool LoadDaySetup(datetime t,const MqlRates &cur)
{
   ...
   if(CopyRates(_Symbol,PERIOD_D1,shift+1,7,refs)!=7)
   {
      g_day_reference_unusable=true;
      g_reference_unusable_days++;
      PrintFormat("D038 V100 SKIP_REFERENCE_UNAVAILABLE | symbol=%s | day=%s",_Symbol,TimeToString(cur.time,TIME_DATE));
      return true;
   }
   ...
   for(int i=0;i<7;i++)
   {
         ...
         !ValidPrice(refs[i].high) || !ValidPrice(refs[i].low) || refs[i].high<=refs[i].low)
      {
         g_day_reference_unusable=true;
         g_reference_unusable_days++;
         PrintFormat("D038 V100 SKIP_REFERENCE_UNUSABLE | symbol=%s | day=%s | ref_index=%d | ref_time=%s | high=%.10f | low=%.10f",
   ...
   g_nr7_high=refs[0].high;
   g_nr7_low=refs[0].low;
   g_nr7_range=ranges[0];
   g_setup_active=true;
   g_nr7_setup_days++;
   return true;
}
...
void WriteStats(string status)
{
   ...
   FileWrite(g_stats_fh,status,SOURCE_NAME,SOURCE_VERSION,RUN_TOKEN,
      g_ticks_seen,g_days_initialized,g_nr7_setup_days,g_non_nr7_days,g_reference_unusable_days,g_ambiguous_days,
...
void CloseTrade(datetime exit_time,long exit_msc,double exit_px,string why)
{
   ...
   if(g_trades_fh!=INVALID_HANDLE)
   {
      FileWrite(g_trades_fh,
         ...
         DoubleToString(g_nr7_high,_Digits),DoubleToString(g_nr7_low,_Digits),DoubleToString(g_nr7_range,_Digits),
...
void OpenTrade(const MqlTick &tick,bool long_side)
{
   ...
   g_stop=g_long ? g_nr7_low : g_nr7_high;
...
void EvaluateEntry(const MqlTick &tick)
{
   ...
   bool long_hit=(tick.ask>=g_nr7_high);
   bool short_hit=(tick.bid<=g_nr7_low);
   if(long_hit && short_hit)
   {
      g_day_ambiguous=true;
      g_ambiguous_days++;
      PrintFormat("D038 V100 AMBIGUOUS SAME TICK | symbol=%s | day=%d | bid=%.10f | ask=%.10f | high=%.10f | low=%.10f",
                  _Symbol,g_day_key,tick.bid,tick.ask,g_nr7_high,g_nr7_low);
      return;
   }
...
int OnInit()
{
   ...
   {
      PrintFormat("D038 V100 FATAL wrong timeframe | got=%s | required=PERIOD_M15",EnumToString(_Period));
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!InpWriteCSV)
   {
      Print("D038 V100 FATAL InpWriteCSV must remain true.");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!IsAllowedSymbol())
   {
      PrintFormat("D038 V100 FATAL symbol outside frozen universe | %s",_Symbol);
      return INIT_PARAMETERS_INCORRECT;
   }
   if(AccountInfoString(ACCOUNT_CURRENCY)!="USD")
   {
      PrintFormat("D038 V100 FATAL account currency must be USD | got=%s",AccountInfoString(ACCOUNT_CURRENCY));
      return INIT_PARAMETERS_INCORRECT;
   }

   string sym=CleanSymbol();
   g_stats_name=StringFormat("D038_V100_RUN_%s_STATS.csv",sym);
   g_trades_name=StringFormat("D038_V100_RUN_%s_TRADES.csv",sym);
   ...
   {
      PrintFormat("D038 V100 FATAL cannot open STATS | err=%d | %s%s",GetLastError(),g_common_path,g_stats_name);
      return INIT_FAILED;
   }
   FileWrite(g_stats_fh,"status","source_name","source_version","run_stage","ticks_seen","days_initialized","nr7_setup_days",
      "non_nr7_days","reference_unusable_days","ambiguous_days","entry_signals","trades_opened","trades_closed","stop_exits",
   ...
   if(g_trades_fh==INVALID_HANDLE)
   {
      ...
      PrintFormat("D038 V100 FATAL cannot open TRADES | err=%d | %s%s",GetLastError(),g_common_path,g_trades_name);
      FileClose(g_stats_fh); g_stats_fh=INVALID_HANDLE;
      return INIT_FAILED;
   }

   FileWrite(g_trades_fh,
      "run_stage","symbol","asset_class","commission_rule","day_key","broker_day_start","nr7_high","nr7_low","nr7_range",
   ...
   ResetDay();
   ResetTrade();
   PrintFormat("D038 V100 READY | source=%s | symbol=%s | Model0 tick execution | NR7 | native Trade Path | NO ORDERS",SOURCE_NAME,_Symbol);
...
void OnDeinit(const int reason)
{
   ...
   PrintFormat("D038 V100 FINAL | status=%s | symbol=%s | ticks=%I64d | nr7_days=%I64d | opened=%I64d | closed=%I64d | rows=%I64d | path_rows=%I64d | invalid_price=%I64d | invalid_risk=%I64d | pnl_failures=%I64d | path_failures=%I64d",
      final_status,_Symbol,g_ticks_seen,g_nr7_setup_days,g_trades_opened,g_trades_closed,g_csv_rows,g_path_rows,g_invalid_price,g_invalid_risk,g_pnl_calc_failures,g_path_calc_failures);
}
