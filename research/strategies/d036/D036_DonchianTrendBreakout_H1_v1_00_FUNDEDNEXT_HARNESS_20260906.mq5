...
#property description "D036 Donchian/Turtle-inspired H1 trend breakout V0 - FundedNext research harness - no orders"
...
string SOURCE_NAME="D036_DonchianTrendBreakout_H1_v1_00_FUNDEDNEXT_HARNESS_20260906.mq5";
...
int OnInit()
{
   ...
   {
      PrintFormat("D036 V100 FATAL wrong timeframe | got=%s | required=PERIOD_H1",EnumToString(_Period));
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!InpWriteCSV)
   {
      Print("D036 V100 FATAL InpWriteCSV must remain true.");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!IsAllowedSymbol())
   {
      PrintFormat("D036 V100 FATAL symbol outside frozen universe | %s",_Symbol);
      return INIT_PARAMETERS_INCORRECT;
   }
   string account_ccy=AccountInfoString(ACCOUNT_CURRENCY);
   if(account_ccy!="USD")
   {
      PrintFormat("D036 V100 FATAL account currency must be USD for frozen commission model | got=%s",account_ccy);
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   {
      PrintFormat("D036 V100 FATAL iATR handle failed | err=%d",GetLastError());
      return INIT_FAILED;
   }

   string sym=CleanSymbol();
   string stage=StageName();
   g_stats_name=StringFormat("D036_V100_%s_%s_STATS.csv",stage,sym);
   g_trades_name=StringFormat("D036_V100_%s_%s_TRADES.csv",stage,sym);
   ...
   {
      PrintFormat("D036 V100 FATAL cannot open STATS | err=%d | %s%s",GetLastError(),g_common_path,g_stats_name);
      return INIT_FAILED;
   }
   FileWrite(g_stats_fh,"status","source_name","source_version","run_stage","bars_seen","bars_in_stage","breakout_signals","trades_opened","trades_closed","stop_exits","channel_exits","stage_end_exits","test_end_exits","invalid_risk","csv_trade_rows","symbol","timeframe","asset_class","commission_rule","trades_name","stats_name","trades_csv_fullpath","stats_csv_fullpath");
   WriteStats("INIT");
   ...
   if(g_trades_fh==INVALID_HANDLE)
   {
      ...
      PrintFormat("D036 V100 FATAL cannot open TRADES | err=%d | %s%s",GetLastError(),g_common_path,g_trades_name);
   ...
   PrintFormat("D036 V100 READY | source=%s | stage=%s | symbol=%s | H1 | class=%s | NO ORDERS",SOURCE_NAME,stage,_Symbol,AssetClass());
...
void OnDeinit(const int reason)
{
   ...
   PrintFormat("D036 V100 FINAL | stage=%s | symbol=%s | trades=%d | rows=%d | stops=%d | channels=%d",StageName(),_Symbol,g_trades_closed,g_csv_rows,g_stop_exits,g_channel_exits);
}
