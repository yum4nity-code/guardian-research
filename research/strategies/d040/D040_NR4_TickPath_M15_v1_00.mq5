...
#property description "D040 NR4 volatility contraction breakout V0 - tick execution and native Trade Path telemetry"

// D040 V0 frozen research harness. NO ORDERS.
// Preregistration:
// research/campaigns/D040_NR4_VOLATILITY_CONTRACTION_BREAKOUT_V0_PREREGISTRATION_2026_09_07.md
// Trade Path contract:
// research/runner/TRADE_PATH_DATASET_SPEC.md
//
// Frozen Guardian adaptation:
// - previous completed broker D1 bar must be strict NR4: its positive range is
//   strictly smaller than each of D-2, D-3 and D-4; ties do not qualify
// - next broker day: first executable tick break of NR4 high/low wins
// - long entry at observed ask; short entry at observed bid
// - initial stop at opposite NR4 extreme
// - one trade/setup/day, no reversal
// - no TP, trailing, BE, partial, indicator, direction or regime filter
// - exit stop, last tick before broker-day change, or tester end
// - frozen FundedNext cost model + 1.5x commission stress
// - native executable-side MFE/MAE and R milestone telemetry
//
// Stage selection is NOT encoded in the EA. Tester FromDate/ToDate selects
// smoke/development/confirmation; output run_stage token remains RUN.

input bool InpWriteCSV=true;

string EXPERIMENT_ID="D040-NR4-VOLATILITY-CONTRACTION-BREAKOUT-V0";
string SOURCE_NAME="D040_NR4_TickPath_M15_v1_00.mq5";
...
double g_nr4_high=0.0;
double g_nr4_low=0.0;
double g_nr4_range=0.0;
...
long g_nr4_setup_days=0;
long g_non_nr4_days=0;
...
void SetFatal(string status,string message)
{
   ...
   PrintFormat("D040 V100 FATAL | %s | %s",status,message);
}
...
bool RiskMoney(double entry_px,double stop_px,bool long_side,double &risk_money)
{
   ...
   {
      PrintFormat("D040 V100 RISK CALC FAILED | symbol=%s | err=%d | entry=%.10f | stop=%.10f",_Symbol,err,entry_px,stop_px);
      return false;
   }
...
bool PnlMoney(double entry_px,double exit_px,bool long_side,double &pnl)
{
   ...
   {
      PrintFormat("D040 V100 PNL CALC FAILED | symbol=%s | err=%d | entry=%.10f | exit=%.10f",_Symbol,err,entry_px,exit_px);
      return false;
   }
...
bool PathR(double exit_px,double &r)
{
   ...
   {
      PrintFormat("D040 V100 PATH CALC FAILED | symbol=%s | err=%d | entry=%.10f | exit=%.10f",_Symbol,err,g_entry,exit_px);
      return false;
   }
...
void ResetDay()
{
   g_day_start=0; g_day_key=0; g_setup_active=false; g_day_traded=false; g_day_ambiguous=false;
   g_day_reference_unusable=false; g_nr4_high=0.0; g_nr4_low=0.0; g_nr4_range=0.0;
}
...
bool LoadDaySetup(datetime t,const MqlRates &cur)
{
   ...
   if(CopyRates(_Symbol,PERIOD_D1,shift+1,4,refs)!=4)
   {
      g_day_reference_unusable=true;
      g_reference_unusable_days++;
      PrintFormat("D040 V100 SKIP_REFERENCE_UNAVAILABLE | symbol=%s | day=%s",_Symbol,TimeToString(cur.time,TIME_DATE));
      return true;
   }
   ...
   for(int i=0;i<4;i++)
   {
      if(refs[i].time<=0 || refs[i].time>=cur.time || !ValidPrice(refs[i].high) || !ValidPrice(refs[i].low) || refs[i].high<=refs[i].low)
      {
         g_day_reference_unusable=true;
         g_reference_unusable_days++;
         PrintFormat("D040 V100 SKIP_REFERENCE_UNUSABLE | symbol=%s | day=%s | ref_index=%d | ref_time=%s | high=%.10f | low=%.10f",
                     _Symbol,TimeToString(cur.time,TIME_DATE),i+1,TimeToString(refs[i].time,TIME_DATE),refs[i].high,refs[i].low);
         return true;
      }
      ranges[i]=refs[i].high-refs[i].low;
      if(!(ranges[i]>0.0) || !MathIsValidNumber(ranges[i]))
      {
         g_day_reference_unusable=true;
         g_reference_unusable_days++;
         return true;
   ...
   for(int i=1;i<4;i++)
   {
      if(!(ranges[0]<ranges[i])){nr4=false;break;}
   }
   if(!nr4)
   {
      g_non_nr4_days++;
   ...
   g_nr4_high=refs[0].high;
   g_nr4_low=refs[0].low;
   g_nr4_range=ranges[0];
   g_setup_active=true;
   g_nr4_setup_days++;
   return true;
}
...
void WriteStats(string status)
{
   ...
   FileWrite(g_stats_fh,status,SOURCE_NAME,SOURCE_VERSION,RUN_TOKEN,
      g_ticks_seen,g_days_initialized,g_nr4_setup_days,g_non_nr4_days,g_reference_unusable_days,g_ambiguous_days,
...
void CloseTrade(datetime exit_time,long exit_msc,double exit_px,string why)
{
   ...
   if(g_trades_fh!=INVALID_HANDLE)
   {
      FileWrite(g_trades_fh,
         ...
         DoubleToString(g_nr4_high,_Digits),DoubleToString(g_nr4_low,_Digits),DoubleToString(g_nr4_range,_Digits),
...
void OpenTrade(const MqlTick &tick,bool long_side)
{
   ...
   g_stop=g_long ? g_nr4_low : g_nr4_high;
...
void EvaluateStop(const MqlTick &tick)
{
   ...
   if(g_long)
   {
      if(tick.bid<=g_stop) CloseTrade(tick.time,TickMsc(tick),tick.bid,"STOP");
   }
   else
   {
      if(tick.ask>=g_stop) CloseTrade(tick.time,TickMsc(tick),tick.ask,"STOP");
   }
}
...
void EvaluateEntry(const MqlTick &tick)
{
   ...
   bool long_hit=(tick.ask>=g_nr4_high);
   bool short_hit=(tick.bid<=g_nr4_low);
   if(long_hit && short_hit)
   {
      g_day_ambiguous=true;
      g_ambiguous_days++;
      PrintFormat("D040 V100 AMBIGUOUS SAME TICK | symbol=%s | day=%d | bid=%.10f | ask=%.10f | high=%.10f | low=%.10f",
                  _Symbol,g_day_key,tick.bid,tick.ask,g_nr4_high,g_nr4_low);
      return;
   }
...
int OnInit()
{
   ...
   {
      PrintFormat("D040 V100 FATAL wrong timeframe | got=%s | required=PERIOD_M15",EnumToString(_Period));
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!InpWriteCSV)
   {
      Print("D040 V100 FATAL InpWriteCSV must remain true.");
      return INIT_PARAMETERS_INCORRECT;
   }
   if(!IsAllowedSymbol())
   {
      PrintFormat("D040 V100 FATAL symbol outside frozen universe | %s",_Symbol);
      return INIT_PARAMETERS_INCORRECT;
   }
   if(AccountInfoString(ACCOUNT_CURRENCY)!="USD")
   {
      PrintFormat("D040 V100 FATAL account currency must be USD | got=%s",AccountInfoString(ACCOUNT_CURRENCY));
      return INIT_PARAMETERS_INCORRECT;
   }

   string sym=CleanSymbol();
   g_stats_name=StringFormat("D040_V100_RUN_%s_STATS.csv",sym);
   g_trades_name=StringFormat("D040_V100_RUN_%s_TRADES.csv",sym);
   ...
   {
      PrintFormat("D040 V100 FATAL cannot open STATS | err=%d | %s%s",GetLastError(),g_common_path,g_stats_name);
      return INIT_FAILED;
   }
   FileWrite(g_stats_fh,"status","source_name","source_version","run_stage","ticks_seen","days_initialized","nr4_setup_days",
      "non_nr4_days","reference_unusable_days","ambiguous_days","entry_signals","trades_opened","trades_closed","stop_exits",
   ...
   if(g_trades_fh==INVALID_HANDLE)
   {
      ...
      PrintFormat("D040 V100 FATAL cannot open TRADES | err=%d | %s%s",GetLastError(),g_common_path,g_trades_name);
      FileClose(g_stats_fh); g_stats_fh=INVALID_HANDLE;
      return INIT_FAILED;
   }

   FileWrite(g_trades_fh,
      "run_stage","symbol","asset_class","commission_rule","day_key","broker_day_start","nr4_high","nr4_low","nr4_range",
   ...
   ResetDay();
   ResetTrade();
   PrintFormat("D040 V100 READY | source=%s | symbol=%s | Model0 tick execution | strict NR4 | native Trade Path | NO ORDERS",SOURCE_NAME,_Symbol);
...
void OnDeinit(const int reason)
{
   ...
   PrintFormat("D040 V100 FINAL | status=%s | symbol=%s | ticks=%I64d | nr4_days=%I64d | opened=%I64d | closed=%I64d | rows=%I64d | path_rows=%I64d | invalid_price=%I64d | invalid_risk=%I64d | pnl_failures=%I64d | path_failures=%I64d",
      final_status,_Symbol,g_ticks_seen,g_nr4_setup_days,g_trades_opened,g_trades_closed,g_csv_rows,g_path_rows,g_invalid_price,g_invalid_risk,g_pnl_calc_failures,g_path_calc_failures);
}
