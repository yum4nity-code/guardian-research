...
def simulate(d,r):
 ...
 need=max(ch,vw+BASELINE_HOURS+1)
 for i in range(need,len(d)):
  t=d.time.iloc[i]
  if next_free is not None and t<next_free: continue
  hist_start=min(t-pd.Timedelta(hours=ch),t-pd.Timedelta(hours=vw+BASELINE_HOURS+1))
  hs=idx.get(hist_start)
  ...
  out.append({'decision_time':t,'entry_time':et,'exit_time':xt,'direction':direction,'vol_ratio':vr,'channel_high':hi,'channel_low':lo,'gross':gross,'E1':gross-COSTS['E1'],'STRESS':gross-COSTS['STRESS']})
  next_free=xt
 return out
...
def main():
 ...
 result={'schema':1,'method':'btc_eth_volatility_compression_channel_breakout','generated_at_utc':datetime.now(timezone.utc).isoformat(),'preregistration':'research/autonomous/R12_BTC_ETH_COMPRESSION_BREAKOUT_PREREGISTRATION_2026_09_13.md','protected_2026_opened':False,'definitions_tested':total,'input_sha256':hashes,'baseline_hours':BASELINE_HOURS,'discovery_candidate_count':len(disc),'discovery_frozen_ids_sha256':dh,'confirmation_candidate_count':len(conf),'confirmation_frozen_ids_sha256':chash,'survivor_count':len(surv),'survivors':surv}
 rp=out/'r12_btc_eth_compression_breakout_result.json'; cp=out/'r12_btc_eth_compression_breakout_survivors.csv'; atomic_json(rp,result); pd.json_normalize(surv).to_csv(cp,index=False)
 hb(prog,total,total,'complete',{'survivors':len(surv)}); status='PASS' if surv else 'FAIL'; summary=f'R12 BTC/ETH compression breakout {status}: {len(surv)} pre-OOS survivors after frozen 2018-22 discovery, 2023-24 confirmation and 2025 gate; 2026 untouched.'; publish(A.publisher,status,summary,[rp,cp]); print(json.dumps({'status':status,'survivors':len(surv),'protected_2026_opened':False})); return 0
