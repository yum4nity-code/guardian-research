...
def load_mt5(path:Path):
    ...
    if not req.issubset(z.columns): raise RuntimeError(f'bad MT5 trade csv: {path}')
...
def nearest_offsets_hours(canon:pd.Series, mt5:pd.Series):
    if canon.empty or mt5.empty:return []
    m=mt5.astype('int64').to_numpy()
    out=[]
    for ts in canon:
        x=int(ts.value)
        j=int(abs(m-x).argmin())
        out.append(round((m[j]-x)/3_600_000_000_000,6))
    return out


def summarize_offsets(vals):
    if not vals:return {'count':0,'mode_hours':None,'mode_count':0,'top_offsets_hours':[]}
    rounded=[round(v,3) for v in vals]
    c=Counter(rounded)
    top=c.most_common(10)
    return {'count':len(vals),'mode_hours':top[0][0],'mode_count':top[0][1],'top_offsets_hours':[{'hours':k,'count':v} for k,v in top]}
...
def exact_after_shift(canon:pd.DataFrame, mt5:pd.DataFrame, hours:float):
    ...
    e={a+d for a in canon.entry_time}; me=set(mt5.entry_time)
    return {'shift_hours':hours,'exact_entry_matches':len(e&me),'exact_pair_matches':len(c&m),'canonical_trades':len(canon),'mt5_trades':len(mt5)}
...
def main():
    ...
    args=ap.parse_args()
    ib=Path(args.phase_ib_root); mt5dir=Path(args.mt5_dir); out=Path(args.output)
    ...
    result={'schema':1,'phase':'top2-mt5-time-alignment-diagnostic','generated_at_utc':datetime.now(timezone.utc).isoformat(),'protected_2026_opened':False,'canonical_inputs':r6.EXPECTED,'purpose':'Diagnose infrastructure timestamp alignment only; frozen R6 rules are unchanged.','candidates':{}}
    for cid in CANDIDATES:
        ...
        for year in YEARS:
            ...
            mode=sm['mode_hours']
            shifts=[]
            for h in sorted(set([-3,-2,-1,0,1,2,3]+([mode] if mode is not None else []))):
                shifts.append(exact_after_shift(c,m,float(h)))
            rec['years'][str(year)]={'nearest_entry_offset_summary':sm,'shift_match_table':shifts}
        result['candidates'][cid]=rec
    atomic_json(out,result)
    print(json.dumps({'status':'PASS','protected_2026_opened':False,'output':str(out)}))
    return 0
