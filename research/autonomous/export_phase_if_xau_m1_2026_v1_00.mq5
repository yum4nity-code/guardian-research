...
void OnStart()
{
   ...
   datetime to   = D'2026.09.01 00:00:00'; // exclusive by filtering below
   MqlRates rates[];
   ArraySetAsSeries(rates,false);
   ResetLastError();
   int n=CopyRates(InpSymbol,PERIOD_M1,from,to,rates);
   if(n<=0){ PrintFormat("I-F XAU EXPORT FAIL CopyRates=%d err=%d",n,GetLastError()); return; }
   string rel="Guardian\\phase_if\\xauusd_m1_2026_jan_aug.csv";
   int h=FileOpen(rel,FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON,',');
   if(h==INVALID_HANDLE){ PrintFormat("I-F XAU EXPORT FAIL FileOpen err=%d",GetLastError()); return; }
   ...
   PrintFormat("I-F XAU EXPORT COMPLETE rows=%d output=%s",written,rel);
}
