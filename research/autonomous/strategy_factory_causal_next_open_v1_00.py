...
def inventory(roots,max_files):
    ...
    for root in roots:
        ...
        for f in p.rglob('*.csv'):
            ...
            # This factory must never inspect known protected/OOS files, even for headers.
            name=str(f).lower()
            if '2026' in name or 'phase_if' in name or 'oos' in name:continue
...
def load_market(path,m,max_rows):
    ...
    # Reject a file outright if any protected row is present; do not silently trim a mixed file.
    if (z.time>=PROTECTED).any():
        raise RuntimeError(f'protected rows present in input: {path}')
...
def causal_return(df,atr,h,direction):
    # Signal is evaluated after bar t closes. Earliest causal fill proxy is open[t+1].
    # A horizon of h bars exits at open[t+h+1], so no bar-t close is captured as PnL.
    ent=df.open.shift(-1); ex=df.open.shift(-(h+1)); ret=((ex-ent)/atr*direction).to_numpy(float)
    ...
    valid=np.isfinite(ret) & (signal_year==entry_year) & (entry_year==exit_year) & np.isin(signal_year,[2024,2025])
    ret[~valid]=np.nan
    return ret

def welch_edge(selected,base,ret,min_n=100):
    valid=base & np.isfinite(ret); a=ret[selected & valid]; b=ret[(~selected) & valid]
...
def hac_edge(selected,base,ret,max_lag,min_n=100):
    valid=base & np.isfinite(ret); g=selected[valid].astype(float); y=ret[valid]; n=len(y); ns=int(g.sum()); nb=n-ns
    if ns<min_n or nb<min_n:return None
    pg=float(g.mean()); edge=float(y[g==1].mean()-y[g==0].mean()); z=y*(g/pg-(1.0-g)/(1.0-pg)); z=z-z.mean(); n=len(z)
    gamma0=float(np.dot(z,z)/n); lrv=gamma0; L=min(int(max_lag),n-1)
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--roots',nargs='+',required=True); ap.add_argument('--output-dir',required=True); ap.add_argument('--progress-file'); ap.add_argument('--trials',type=int,default=150000); ap.add_argument('--max-files',type=int,default=20); ap.add_argument('--max-rows',type=int,default=900000); ap.add_argument('--seed',type=int,default=260914); ap.add_argument('--publisher')
    ...
    if not markets:raise RuntimeError('no eligible strictly pre-2026 2024+2025 dataset')
    discovery=[]; seen=set()
    for i in range(args.trials):
        ...
        cut=float(np.quantile(x24,q)); rule={'market_index':mi,'dataset':M['path'],'feature':feature,'operator':op,'quantile':q,'cutpoint':cut,'horizon_bars':h,'direction':direction,'hour_start':hour_start,'hour_width':hour_width,'execution':'signal_after_close_entry_next_open_exit_open_after_h_bars'}
        sel=apply_rule(df,ft,rule); base=base_mask(df,rule); ret=causal_return(df,atr,h,direction); s24=welch_edge(sel&(y==2024),base&(y==2024),ret,100)
        if s24 and s24['edge_atr']>0.02 and s24['p_fast']<0.01:rule['y2024']=s24; discovery.append(rule)
        if (i+1)%1000==0:hb(progress,i+1,args.trials,'discovery_2024_causal_next_open',{'datasets':len(markets),'discovery_candidates':len(discovery)})
    confirms=[]; pvals=[]; totalc=len(discovery)
    for j,r in enumerate(discovery):
        M=markets[r['market_index']]; df=M['df']; ft=M['ft']; atr=M['atr']; y=df.time.dt.year.to_numpy(); mo=df.time.dt.month.to_numpy(); h=r['horizon_bars']; sel=apply_rule(df,ft,r); base=base_mask(df,r); ret=causal_return(df,atr,h,r['direction'])
        s25=welch_edge(sel&(y==2025),base&(y==2025),ret,100); quarters=[]
        for qn,(a,b) in enumerate(((1,3),(4,6),(7,9),(10,12)),1):quarters.append(welch_edge(sel&(y==2025)&(mo>=a)&(mo<=b),base&(y==2025)&(mo>=a)&(mo<=b),ret,30))
        hac=None
        if s25 and all(quarters) and s25['edge_atr']>0.015 and s25['p_fast']<0.01 and all(qx['edge_atr']>0 for qx in quarters):hac=hac_edge(sel&(y==2025),base&(y==2025),ret,max_lag=max(48,2*h),min_n=100)
        p=1.0 if hac is None else hac['p_hac']; pvals.append(p); rr=dict(r); rr['y2025_fast']=s25; rr['y2025_quarters']=quarters; rr['y2025_hac']=hac; confirms.append(rr)
        if (j+1)%250==0:hb(progress,j+1,max(1,totalc),'confirmation_2025_causal_next_open',{'discovery_candidates':totalc})
    qvals=bh_qvalues(np.array(pvals,float)) if pvals else np.array([]); survivors=[]
    for idx,r in enumerate(confirms):
        r['confirmation_bh_q']=float(qvals[idx]) if len(qvals) else 1.0; s=r['y2025_hac']; qs=r['y2025_quarters']
        if not (s and all(qs) and r['confirmation_bh_q']<=0.05 and s['edge_atr']>0.015 and all(qx['edge_atr']>0 for qx in qs)):continue
        qi=ALL_Q.index(r['quantile']); neigh=[k for k in [qi-1,qi+1] if 0<=k<len(ALL_Q) and ((r['operator']=='lt' and ALL_Q[k]<=.30) or (r['operator']=='gt' and ALL_Q[k]>=.70))]; robust=False; nstats=[]
        M=markets[r['market_index']]; df=M['df']; ft=M['ft']; atr=M['atr']; y=df.time.dt.year.to_numpy(); x=ft[r['feature']].to_numpy(float); x24=x[(y==2024)&np.isfinite(x)]; ret=causal_return(df,atr,r['horizon_bars'],r['direction']); base=base_mask(df,r)
        for k in neigh:
            nq=ALL_Q[k]; nc=float(np.quantile(x24,nq)); nm=apply_rule(df,ft,r,cut_override=nc); ns=welch_edge(nm&(y==2025),base&(y==2025),ret,80); nstats.append({'quantile':nq,'cutpoint':nc,'y2025_edge':ns})
            if ns and ns['edge_atr']>0.0075:robust=True
        r['neighbor_stress']=nstats
        if robust:r.pop('market_index',None); survivors.append(r)
    survivors.sort(key=lambda r:(r['confirmation_bh_q'],-min([r['y2024']['edge_atr'],r['y2025_hac']['edge_atr']]+[q['edge_atr'] for q in r['y2025_quarters']]))); survivors=survivors[:500]
    result={'schema':1,'method':'causal_next_open_conditional_edge_factory','generated_at_utc':datetime.now(timezone.utc).isoformat(),'protected_2026_untouched':True,'signal_availability':'after source bar close','entry_proxy':'next available bar open','exit_proxy':'open after h holding bars','calendar_boundary_purge':True,'thresholds_fit_on_2024_only':True,'confirmation_year':2025,'multiple_testing':'Benjamini-Hochberg FDR 5% across all 2024 discovery candidates','trials':args.trials,'datasets_scanned':len(files),'datasets_eligible':len(markets),'unique_rules_tested':len(seen),'discovery_candidate_count':len(discovery),'survivor_count':len(survivors),'survivors':survivors}
    rp=out/'strategy_factory_causal_result.json'; atomic_json(rp,result); cp=out/'strategy_factory_causal_survivors.csv'; (pd.json_normalize(survivors) if survivors else pd.DataFrame()).to_csv(cp,index=False)
    ...
    status='PASS' if survivors else 'FAIL'; summary=f'{len(survivors)} causal next-open survivors after 2024 discovery, untouched 2025 HAC/BH-FDR confirmation, four-quarter and adjacent-quantile robustness; 2026 never opened.'
    if args.publisher:publish(args.publisher,'strategy-factory-r5-causal-next-open',status,summary,[rp,cp,out/'dataset_inventory.json'])
    print(json.dumps({'status':status,'trials':args.trials,'discovery_candidates':len(discovery),'survivors':len(survivors),'protected_2026_untouched':True},sort_keys=True)); return 0
