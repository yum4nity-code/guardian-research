...
import audit_r5_post_result_execution_semantics_v1_02 as audit
...
def test_timestamp_resolution_independence():
    ...
    actual_us = audit.raw_m1_reference_return(src_us, atr, raw_us, horizon=1, direction=1, tf_seconds=300)
    assert np.allclose(expected, actual_us, equal_nan=True)

    raw_ms = raw.copy(); src_ms = src.copy()
    raw_ms['time'] = pd.Series(raw_ms['time'].array.as_unit('ms'))
    src_ms['time'] = pd.Series(src_ms['time'].array.as_unit('ms'))
    actual_ms = audit.raw_m1_reference_return(src_ms, atr, raw_ms, horizon=1, direction=1, tf_seconds=300)
    assert np.allclose(expected, actual_ms, equal_nan=True)
