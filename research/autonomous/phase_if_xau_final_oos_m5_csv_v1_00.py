...
def main():
 ...
 if not csvp.exists() or not manp.exists(): raise RuntimeError('MT5 XAU M5 snapshot missing')
 ...
 md=manifest(manp)
 if md.get('symbol')!='XAUUSD' or md.get('period')!='M5' or md.get('from')!='2026.01.01 00:00:00' or md.get('to_exclusive')!='2026.09.01 00:00:00': raise RuntimeError(f'invalid snapshot manifest: {md}')
 ...
 hb(prog,10,100,'read_mt5_m5_snapshot'); df=pd.read_csv(csvp)
 ...
 if len(df)<30000: raise RuntimeError(f'too few M5 rows: {len(df)}')
 if int(df.server_epoch.min())>int(datetime(2026,1,7,tzinfo=timezone.utc).timestamp()) or int(df.server_epoch.max())<int(datetime(2026,8,25,tzinfo=timezone.utc).timestamp()): raise RuntimeError('incomplete M5 frozen-window coverage')
 hb(prog,30,100,'news_mask'); clean=mask_clean(df,a.news_mask)
 ...
 status='PASS' if all(gates.values()) else 'FAIL'; out=Path(a.output_dir); out.mkdir(parents=True,exist_ok=True)
 result={'schema':1,'phase':'I-F','status':status,'generated_at_utc':datetime.now(timezone.utc).isoformat(),'candidate':c,'snapshot_provenance':{'csv':str(csvp),'csv_sha256':sha(csvp),'manifest':str(manp),'manifest_sha256':sha(manp),'manifest_fields':md,'m5_rows_raw':len(df),'m5_rows_news_clean':len(clean),'outcomes_after_2026_09_01_decoded':False,'transport':'native_mt5_m5'},'news_mask_sha256':sha(a.news_mask),'full':full,'subperiods':sub,'bootstrap':{'ci_low':lo,'ci_high':hi,'valid_reps':nboot,'replicates':5000,'block_length_bars':24,'seed':20260909},'gates':gates,'all_gates_pass':all(gates.values()),'no_retuning_performed':True,'decision':'promote_to_execution_translation' if status=='PASS' else 'close_xau_directional_candidate_family'}
 rp=out/'phase_if_final_oos_result.json'; atomic(rp,result); hb(prog,100,100,'complete'); publish(Path(a.publisher) if a.publisher else None,status,f"Phase I-F frozen XAU final OOS via native MT5 M5 snapshot -> {status}; effect={full['effect']:.6f} ATR n={full['n_state']} CI=[{lo:.6f},{hi:.6f}]",[rp,a.policy,a.news_mask,manp],Path(a.deploy)); return 0 if status=='PASS' else 10
