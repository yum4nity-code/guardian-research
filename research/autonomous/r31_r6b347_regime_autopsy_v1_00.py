...
def load_year(rec,tf,year):
    p=Path(rec[f"{tf.lower()}_path"]); z=pd.read_csv(p)
    t=pd.to_datetime(pd.to_numeric(z.server_epoch).astype('int64'),unit='s',utc=True)
    return pd.DataFrame({"time":t,"open":z.open.astype(float),"high":z.high.astype(float),"low":z.low.astype(float),"close":z.close.astype(float)})
...
def cap(trades,profile='E1',initial=10000.0):
    ...
    for t in sorted(trades,key=lambda x:x['entry_time']):
        ...
        e*=1+r; peak=max(peak,e); dd=max(dd,(peak-e)/peak)
    return {"ending":e,"return_pct":(e/initial-1)*100,"max_dd_pct":dd*100,"trades":len(trades)}

def attach_features(allm5):
    c=allm5.close.astype(float)
    lr=np.log(c).diff()
    for d in (20,60,252): allm5[f'ret{d}']=c/c.shift(BPD*d)-1
    allm5['vol20']=lr.rolling(BPD*20,min_periods=BPD*20).std()
    allm5['vol60']=lr.rolling(BPD*60,min_periods=BPD*60).std()
    abs_sum=lr.abs().rolling(BPD*20,min_periods=BPD*20).sum()
    allm5['eff20']=np.log(c/c.shift(BPD*20)).abs()/abs_sum
    allm5['dist_high252']=c/c.shift(1).rolling(BPD*252,min_periods=BPD*252).max()-1
    atr=r6.atr14(allm5)
    allm5['atr_pct']=atr/c
    hi=allm5.high.shift(1).rolling(96,min_periods=96).max()
    allm5['breakout_margin_atr']=(c-hi)/atr
    return allm5
...
def main():
    ...
    out=a.output_dir; out.mkdir(parents=True,exist_ok=True); rp=out/'r31_regime_autopsy.json'
    if rp.exists(): raise RuntimeError('existing R31 result; refusing overwrite')
    years=sorted(int(y) for y in man['yearly'])
    hb(a.progress_file,0,5,'load_m5')
    m5parts=[]
    for y in years:
        m=load_year(man['yearly'][str(y)],'M5',y); m['year']=y; m5parts.append(m)
    allm5=pd.concat(m5parts,ignore_index=True).sort_values('time').reset_index(drop=True)
    hb(a.progress_file,1,5,'features')
    allm5=attach_features(allm5)
    feat_by_time=allm5.set_index('time')

    trades=[]; annual={}
    hb(a.progress_file,2,5,'replay')
    for y in years:
        if y==2004: continue
        ...
        for t in led:
            ...
            if st not in feat_by_time.index: continue
            row=feat_by_time.loc[st]
    ...
    dev=[t for t in trades if 2005<=t['year']<=2017]
    val=[t for t in trades if 2018<=t['year']<=2025]
    hb(a.progress_file,3,5,'scan_univariate')
    scans=[]
    for f in FEATURES:
        ...
        for q in QUANTILES:
            ...
            for op in ('>=','<='):
                keep=lambda t: t[f] is not None and math.isfinite(t[f]) and ((t[f]>=thr) if op=='>=' else (t[f]<=thr))
                d=[t for t in dev if keep(t)]; v=[t for t in val if keep(t)]
                if len(d)<150 or len(v)<80: continue
                de1,ds=cap(d,'E1'),cap(d,'STRESS'); ve1,vs=cap(v,'E1'),cap(v,'STRESS')
                scans.append({"feature":f,"op":op,"quantile":q,"threshold":thr,"dev_E1":de1,"dev_STRESS":ds,"val_E1":ve1,"val_STRESS":vs,
                              "dev_expectancy_bps":econ.stats(d,'E1')['expectancy_bps'],"val_expectancy_bps":econ.stats(v,'E1')['expectancy_bps']})
    scans.sort(key=lambda x:(x['dev_E1']['ending'],x['dev_STRESS']['ending']),reverse=True)
    top=scans[:20]
    ...
    result={"schema":1,"research":"R31","generated_at_utc":datetime.now(timezone.utc).isoformat(),"rule":RULE,
            "development":"2005-2017","validation":"2018-2025","features":FEATURES,"quantiles":QUANTILES,"base":base,
            "annual":annual,"top20_by_dev":top,"validation_survivors_among_top20":survivors,"protected_2026_opened":False,
            "interpretation":"Exploratory univariate causal regime scan. Thresholds chosen on 2005-2017 only; 2018-2025 evaluated after selection."}
    atomic_json(rp,result)
    pd.DataFrame(scans).to_json(out/'r31_all_filters.json',orient='records',indent=2)
    hb(a.progress_file,5,5,'complete',{"survivors":len(survivors),"protected_2026_opened":False})
    print(json.dumps({"status":"PASS","base":base,"top":top[:5],"survivors":survivors,"protected_2026_opened":False},sort_keys=True))
    return 0
