...
ASSETS=['BTCUSDT','ETHUSDT']; LOOKBACKS=[24,72,168,336]; THRESH=[.01,.02,.04]; HOLDS=[24,72,168]
...
def rules(): return [{'asset':a,'lookback_hours':l,'threshold':q,'hold_hours':h} for a in ASSETS for l in LOOKBACKS for q in THRESH for h in HOLDS]
def cid(r): return 'R10-'+hashlib.sha256(json.dumps(r,sort_keys=True,separators=(',',':')).encode()).hexdigest()[:12].upper()
def simulate(d,r):
 idx={t:i for i,t in enumerate(d.time)}; out=[]; next_free=None; lb=r['lookback_hours']; hold=r['hold_hours']; q=r['threshold']
 for i,row in d.iterrows():
  t=row.time
  if t.hour!=0: continue
  if next_free is not None and t<next_free: continue
  lt=t-pd.Timedelta(hours=lb); li=idx.get(lt)
  if li is None: continue
  path_lb=d.time.iloc[li:i+1]
  if len(path_lb)!=(lb+1) or path_lb.diff().dropna().ne(pd.Timedelta(hours=1)).any(): continue
  mom=float(row.close/d.close.iloc[li]-1.0)
  direction=1 if mom>=q else (-1 if mom<=-q else 0)
  ...
  out.append({'decision_time':t,'entry_time':et,'exit_time':xt,'direction':direction,'momentum':mom,'gross':gross,'E1':gross-COSTS['E1'],'STRESS':gross-COSTS['STRESS']})
  next_free=xt
 return out
...
def publish(pub,status,summary,arts):
 if not pub:return
 c=['python',pub,'--phase','r10-btc-eth-multi-day-trend','--status',status,'--summary',summary]
...
def main():
 ...
 R=rules()
 if len(R)!=72: raise RuntimeError('rule count mismatch')
 disc=[]
 ...
 for i,(c,m,y23,y24) in enumerate(temp):
  if m['n']>=30 and m['E1']['mean']>0 and m['STRESS']['mean']>0 and y23['E1']['net_sum']>0 and y24['E1']['net_sum']>0 and qs[i]<=.05:
 ...
 for c in conf:
  ...
  if m['n']>=15 and m['E1']['mean']>0 and m['STRESS']['mean']>0 and h1m['E1']['net_sum']>0 and h2m['E1']['net_sum']>0 and exbest>0 and conc<=.35:
 ...
 result={'schema':1,'method':'btc_eth_multi_day_time_series_trend','generated_at_utc':datetime.now(timezone.utc).isoformat(),'preregistration':'research/autonomous/R10_BTC_ETH_MULTI_DAY_TREND_PREREGISTRATION_2026_09_13.md','protected_2026_opened':False,'definitions_tested':72,'input_sha256':hashes,'discovery_candidate_count':len(disc),'discovery_frozen_ids_sha256':dh,'confirmation_candidate_count':len(conf),'confirmation_frozen_ids_sha256':ch,'survivor_count':len(surv),'survivors':surv}
 rp=out/'r10_btc_eth_multi_day_trend_result.json'; cp=out/'r10_btc_eth_multi_day_trend_survivors.csv'; atomic_json(rp,result); pd.json_normalize(surv).to_csv(cp,index=False)
 hb(prog,72,72,'complete',{'survivors':len(surv)}); status='PASS' if surv else 'FAIL'; summary=f'R10 BTC/ETH multi-day trend {status}: {len(surv)} pre-OOS survivors after frozen 2018-22 discovery, 2023-24 confirmation and 2025 gate; 2026 untouched.'; publish(A.publisher,status,summary,[rp,cp]); print(json.dumps({'status':status,'survivors':len(surv),'protected_2026_opened':False})); return 0
