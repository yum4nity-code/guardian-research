...
HOURS=[0,4,8,12,16,20]; HOLDS=[4,8,12,24]; COSTS={'E1':0.001,'STRESS':0.002}
...
def rules():
 return [{'asset':a,'weekday':w,'hour':h,'hold_hours':k,'direction':s} for a in ASSETS for w in range(7) for h in HOURS for k in HOLDS for s in (-1,1)]
def cid(r): return 'R9-'+hashlib.sha256(json.dumps(r,sort_keys=True,separators=(',',':')).encode()).hexdigest()[:12].upper()
def simulate(d,r):
 idx={t:i for i,t in enumerate(d.time)}; out=[]
 for i,row in d.iterrows():
  t=row.time
  if t.weekday()!=r['weekday'] or t.hour!=r['hour']: continue
  et=t+pd.Timedelta(hours=r['hold_hours'])
  j=idx.get(et)
  if j is None or j<=i: continue
  path=d.time.iloc[i:j+1]
  if len(path)!=(r['hold_hours']+1) or path.diff().dropna().ne(pd.Timedelta(hours=1)).any(): continue
  if t.year!=et.year: continue
  gross=r['direction']*float(d.open.iloc[j]/d.open.iloc[i]-1.0)
  out.append({'entry_time':t,'exit_time':et,'gross':gross,'E1':gross-COSTS['E1'],'STRESS':gross-COSTS['STRESS']})
 return out
def sub(ts,a,b): return [x for x in ts if a<=x['entry_time']<b]
def pval(v):
...
def publish(pub,status,summary,arts):
 if not pub:return
 c=['python',pub,'--phase','r9-btc-eth-calendar-session','--status',status,'--summary',summary]
...
def main():
 ...
 paths={a:Path(A.data_dir)/f'{a}_spot_5m_2017_2025.csv' for a in ASSETS}; data={}; hashes={}
 hb(prog,0,672,'load')
 for a,p in paths.items(): data[a]=h1(load(p)); hashes[p.name]=sha(p)
 R=rules()
 if len(R)!=672: raise RuntimeError('rule count mismatch')
 disc=[]
 for n,r in enumerate(R,1):
  ...
  if m['n']>=180 and m['E1']['mean']>0 and m['STRESS']['mean']>0 and pos>=4 and m['E1']['p']<0.01: disc.append({'candidate_id':cid(r),'rule':r,'discovery':m,'discovery_yearly':yearly,'positive_discovery_years':pos})
  if n%25==0: hb(prog,n,672,'discovery_2018_2022',{'discovery_candidates':len(disc)})
 ids=sorted(x['candidate_id'] for x in disc); dh=hashlib.sha256('|'.join(ids).encode()).hexdigest(); hb(prog,672,672,'discovery_frozen_before_confirmation',{'count':len(ids),'sha256':dh})
 temp=[]; ps=[]
 for c in disc:
 ...
 for i,(c,m,y23,y24) in enumerate(temp):
  if m['n']>=80 and m['E1']['mean']>0 and m['STRESS']['mean']>0 and y23['E1']['net_sum']>0 and y24['E1']['net_sum']>0 and qs[i]<=0.05:
   z=dict(c); z.update({'confirmation':m,'confirmation_2023':y23,'confirmation_2024':y24,'confirmation_bh_q':qs[i]}); conf.append(z)
 ids2=sorted(x['candidate_id'] for x in conf); ch=hashlib.sha256('|'.join(ids2).encode()).hexdigest(); hb(prog,672,672,'confirmation_frozen_before_2025',{'count':len(ids2),'sha256':ch})
 surv=[]
 for c in conf:
  ...
  if m['n']>=40 and m['E1']['mean']>0 and m['STRESS']['mean']>0 and h1m['E1']['net_sum']>0 and h2m['E1']['net_sum']>0 and exbest>0 and conc<=0.35:
 ...
 result={'schema':1,'method':'btc_eth_calendar_session_seasonality','generated_at_utc':datetime.now(timezone.utc).isoformat(),'preregistration':'research/autonomous/R9_BTC_ETH_CALENDAR_SESSION_PREREGISTRATION_2026_09_13.md','protected_2026_opened':False,'definitions_tested':672,'input_sha256':hashes,'discovery_candidate_count':len(disc),'discovery_frozen_ids_sha256':dh,'confirmation_candidate_count':len(conf),'confirmation_frozen_ids_sha256':ch,'survivor_count':len(surv),'survivors':surv}
 rp=out/'r9_btc_eth_calendar_session_result.json'; cp=out/'r9_btc_eth_calendar_session_survivors.csv'; atomic_json(rp,result); pd.json_normalize(surv).to_csv(cp,index=False)
 hb(prog,672,672,'complete',{'survivors':len(surv)}); status='PASS' if surv else 'FAIL'; summary=f'R9 BTC/ETH calendar-session {status}: {len(surv)} pre-OOS survivors after frozen 2018-22 discovery, 2023-24 confirmation and 2025 gate; 2026 untouched.'; publish(A.publisher,status,summary,[rp,cp]); print(json.dumps({'status':status,'survivors':len(surv),'protected_2026_opened':False})); return 0
