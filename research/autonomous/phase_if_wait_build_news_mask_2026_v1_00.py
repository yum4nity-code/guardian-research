...
def main():
 ...
 while time.time()<deadline:
  ...
  if hb: atomic(hb,{'completed':0,'total':1,'stage':'waiting_for_mt5_calendar_export','updated_at_utc':datetime.now(timezone.utc).isoformat()})
  time.sleep(2)
 ...
 atomic(sump,summary)
 if hb: atomic(hb,{'completed':1,'total':1,'stage':'news_mask_frozen','updated_at_utc':datetime.now(timezone.utc).isoformat()})
