...
def heartbeat(p: Path | None, stage: str, elapsed: int, wait_seconds: int, child_pid: int | None = None) -> None:
    if p:
        obj = {
            "completed": 0,
            "total": 1,
        ...
        if child_pid is not None:
            obj["executor_pid"] = child_pid
        write_json_resilient(p, obj)
...
def main() -> int:
    ...
    # Keep an independent liveness heartbeat while the unchanged scientific
    # executor is busy in long HCC parsing/bootstrap stages. This does not
    # inspect or alter scientific outputs.
    child = subprocess.Popen(cmd)
    while True:
        rc = child.poll()
        if rc is not None:
            heartbeat(progress, "executor_finished", int(time.time() - started), a.wait_seconds, child.pid)
            return int(rc)
        heartbeat(progress, "executor_running", int(time.time() - started), a.wait_seconds, child.pid)
        time.sleep(30)
