...
#property description "D035-F1 FTMO BTCUSD/XLMUSD M1 quote exporter. Research only. No orders."

#define EXPERIMENT_ID "D035_FTMO_CFD_M1_EXPORT"
...
int OnInit(){
   string u=StringToUpper(_Symbol);
   if(u!="BTCUSD" && u!="XLMUSD"){
      Print("D035-F1 refuses symbol ",_Symbol,". Only BTCUSD or XLMUSD allowed.");
      return INIT_FAILED;
   }

   g_company=AccountInfoString(ACCOUNT_COMPANY);
   g_server=AccountInfoString(ACCOUNT_SERVER);
   string c=StringToUpper(g_company);
   string s=StringToUpper(g_server);
   if(StringFind(c,"FTMO")<0 && StringFind(s,"FTMO")<0){
      Print("D035-F1 refuses non-FTMO environment. company=",g_company," server=",g_server);
      return INIT_FAILED;
   }

   EnsureFolders();
   g_file=FileOpen(g_run_dir+"\\D035_CFD_M1_"+_Symbol+".csv",FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON,';');
   if(g_file==INVALID_HANDLE){ Print("D035-F1 exporter failed opening file. Error=",GetLastError()); return INIT_FAILED; }
   FileWrite(g_file,
      ...
      "company","server");
   FileFlush(g_file);
   if(FileSize(g_file)<=0){ Print("D035-F1 exporter header QA failed."); return INIT_FAILED; }
   Print("D035-F1 FTMO exporter ready: ",g_run_dir," company=",g_company," server=",g_server);
