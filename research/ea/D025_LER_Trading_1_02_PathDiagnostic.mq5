...
#property description "D025 LER Trading 1.02 - path diagnostic, same locked V0 entries"

#include <Trade/Trade.mqh>

// D025 Liquidity Exhaustion Reclaim
// Trading implementation of the locked V0 signal state machine.
// Single-symbol Strategy Tester EA: trades _Symbol only.
// Entry rules and structural SL are unchanged from Trading 1.01.
// Actual exit remains structural SL or forced close after InpMaxHoldHours. No TP is executed.
// 1.02 adds observational path timestamps only:
//   +0.5R first touch,
//   +1R..+5R first touch,
//   first return to breakeven AFTER +1R,
//   M1 ambiguity flag when ordering cannot be known inside one bar.

input double InpRiskPercent = 0.50;
input int    InpMaxHoldHours = 48;
input ulong  InpMagic        = 25090402;
...
string EventsFile()
{
   return "GuardianResearch\\D025\\d025_ler_trading_1_02_events.csv";
}

string TradesFile()
{
   return "GuardianResearch\\D025\\d025_ler_trading_1_02_trades.csv";
}

string OutcomesFile()
{
   return "GuardianResearch\\D025\\d025_ler_trading_1_02_outcomes.csv";
}
...
void LogOutcome(TrackedTrade &vt,string horizon,datetime utc_time)
{
   ...
   if(h == INVALID_HANDLE) return;
   if(FileSize(h) == 0)
      FileWrite(h,"session_id","event_id","utc_text","symbol","side","horizon","mfe_r","mae_r",
                  "hit05_utc","hit1_utc","hit2_utc","hit3_utc","hit4_utc","hit5_utc","stop_utc",
                  "be_after1_utc","be_after1_ambiguous_same_m1","ambiguous_same_m1");
   FileSeek(h,0,SEEK_END);
   FileWrite(h,g_session_id,vt.event_id,TimeToString(utc_time,TIME_DATE|TIME_SECONDS),_Symbol,SideName(vt.side),horizon,
             ...
             (long)vt.hit05_utc,(long)vt.hit1_utc,(long)vt.hit2_utc,(long)vt.hit3_utc,(long)vt.hit4_utc,(long)vt.hit5_utc,
             (long)vt.stop_utc,(long)vt.be_after1_utc,(vt.be_after1_ambiguous_same_m1 ? "YES" : "NO"),(vt.ambiguous ? "YES" : "NO"));
   FileFlush(h);
   FileClose(h);
}
...
void CreateTrade(LevelTracker &tr,MqlRates &bar,string path,datetime utc_now)
{
   ...
   vt.hit05_utc=0; vt.hit1_utc=0; vt.hit2_utc=0; vt.hit3_utc=0; vt.hit4_utc=0; vt.hit5_utc=0; vt.stop_utc=0;
   vt.be_after1_utc=0; vt.be_after1_ambiguous_same_m1=false;
...
void ProcessTradeBar(TrackedTrade &vt,MqlRates &bar,datetime utc_now)
{
   ...
   if(vt.be_after1_utc==0 && (had_hit1_before || hit1_this_bar))
   {
      ...
      if(be_cross)
      {
         ...
         // If +1R was first touched in this same M1, we do not know whether +1R or BE came first.
         // If +1R was already armed, but a new >=2R target and BE are both inside this M1,
         // we do not know whether the runner target or BE came first.
         if(hit1_this_bar || new_runner_target_hit)
         {
            vt.be_after1_ambiguous_same_m1=true;
            Print("[D025T][AMBIGUOUS_BE_PATH_M1] event=",vt.event_id);
         }
      }
   }
   ...
   if(stop_cross)
   {
      ...
      {
         vt.ambiguous=true;
         Print("[D025T][AMBIGUOUS_M1] event=",vt.event_id);
      }
   }
   ...
   if(!vt.h48_logged && elapsed>=InpMaxHoldHours*3600)
   {
      ...
      Print("[D025T][COMPLETE] event=",vt.event_id,
...
int OnInit()
{
   ...
   g_session_id=StringFormat("D025T102_%s_%I64d_%I64d",_Symbol,(long)TimeGMT(),(long)GetTickCount64());
   ...
   Print("[D025T][START] Trading 1.02 PathDiagnostic symbol=",_Symbol,
         " risk=",DoubleToString(InpRiskPercent,2),"% max_hold=",InpMaxHoldHours,
         "h | SAME_V0_ENTRY | SL_STRUCTURAL | NO_TP | LOG_0.5R_AND_BE_AFTER_1R");
