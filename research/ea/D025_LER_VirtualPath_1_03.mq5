...
#property description "D025 LER Virtual Path 1.03 - no orders, locked V0 entries, path diagnostics"

// D025 Liquidity Exhaustion Reclaim
// Pure virtual Strategy Tester observer.
// - Same locked V0 entry state machine as Trading 1.01/1.02.
// - No CTrade include and no order/position functions.
// - No dependence on balance, margin, lot minimums or execution success.
// - Virtual entry uses current Ask for LONG / Bid for SHORT at VALID_SIGNAL.
...
string EventsFile()
{
   return "GuardianResearch\\D025\\d025_ler_virtual_1_03_events.csv";
}

string TradesFile()
{
   return "GuardianResearch\\D025\\d025_ler_virtual_1_03_trades.csv";
}

string OutcomesFile()
{
   return "GuardianResearch\\D025\\d025_ler_virtual_1_03_outcomes.csv";
}
...
void LogOutcome(TrackedTrade &vt,string horizon,datetime utc_time)
{
   ...
   if(h == INVALID_HANDLE) return;
   if(FileSize(h) == 0)
      FileWrite(h,"session_id","event_id","utc_text","symbol","side","horizon","mfe_r","mae_r","hit05_utc","hit1_utc","hit2_utc","hit3_utc","hit4_utc","hit5_utc","stop_utc","be_after1_utc","be_after1_ambiguous_same_m1","ambiguous_same_m1");
   FileSeek(h,0,SEEK_END);
   FileWrite(h,g_session_id,vt.event_id,TimeToString(utc_time,TIME_DATE|TIME_SECONDS),_Symbol,SideName(vt.side),horizon,
             DoubleToString(vt.mfe_r,6),DoubleToString(vt.mae_r,6),(long)vt.hit05_utc,(long)vt.hit1_utc,(long)vt.hit2_utc,(long)vt.hit3_utc,
             (long)vt.hit4_utc,(long)vt.hit5_utc,(long)vt.stop_utc,(long)vt.be_after1_utc,
...
void TransitionCascade(LevelTracker &tr,MqlRates &bar,double range_shock,double body_atr,double relvol,datetime utc_now)
{
   ...
   if(InpVerbose) Print("[D025V][",_Symbol,"][",LevelName(tr.level_index),"] CASCADE event=",tr.event_id);
}
...
void CreateVirtualTrade(LevelTracker &tr,MqlRates &bar,string path,datetime utc_now)
{
   ...
   double entry=(side==SIDE_LONG ? SymbolInfoDouble(_Symbol,SYMBOL_ASK) : SymbolInfoDouble(_Symbol,SYMBOL_BID));
   if(entry<=0.0) entry=bar.close;
   ...
   if(slot<0)
   {
      ...
      Print("[D025V][NO_SLOT] increase MAX_TRACKED_TRADES if this appears");
      ResetTracker(tr,true);
      return;
   }
   ...
   Print("[D025V][VALID_SIGNAL] event=",tr.event_id," side=",SideName(side)," level=",LevelName(tr.level_index),
...
void FailSequence(LevelTracker &tr,string reason,MqlRates &bar,datetime utc_now)
{
   ...
   if(InpVerbose) Print("[D025V][",_Symbol,"][",LevelName(tr.level_index),"] failed=",reason," event=",tr.event_id);
   ResetTracker(tr,true);
}
...
void ProcessTracker(LevelTracker &tr,MqlRates &bar,MqlRates &prev,double atr,double range_shock,double relvol,datetime utc_now)
{
   ...
   if(tr.state==LER_IDLE || tr.state==LER_WATCH)
   {
      ...
      if(InpVerbose) Print("[D025V][",_Symbol,"][",LevelName(tr.level_index),"] SWEEP event=",tr.event_id," depthATR=",DoubleToString(depth,3));
      double body_atr=0.0;
   ...
   if(tr.state==LER_CASCADE)
   {
      ...
      if(progress_frac<=EXHAUST_PROGRESS_FRAC && activity_ok)
      {
         ...
         if(InpVerbose) Print("[D025V][",_Symbol,"][",LevelName(tr.level_index),"] EXHAUSTION event=",tr.event_id);
         return;
      }
      if(side==SIDE_LONG) tr.last_outward_extreme=MathMin(tr.last_outward_extreme,bar.low); else tr.last_outward_extreme=MathMax(tr.last_outward_extreme,bar.high);
...
void ProcessTradeBar(TrackedTrade &vt,MqlRates &bar,datetime utc_now)
{
   ...
   if(vt.be_after1_utc==0 && (had_hit1_before || hit1_this_bar))
   {
      ...
      if(be_cross)
      {
         ...
         if(hit1_this_bar || new_runner_target_hit)
         {
            ...
            if(InpVerbose) Print("[D025V][AMBIGUOUS_BE_PATH_M1] event=",vt.event_id);
         }
      }
   }
   ...
   if(stop_cross)
   {
      ...
      {
         vt.ambiguous=true;
         if(InpVerbose) Print("[D025V][AMBIGUOUS_M1] event=",vt.event_id);
      }
   }
   ...
   if(!vt.h48_logged && elapsed>=InpMaxHoldHours*3600)
   {
      ...
      Print("[D025V][COMPLETE] event=",vt.event_id,
...
int OnInit()
{
   ...
   {
      Print("[D025V][FATAL] cannot create H1 ATR handle for ",_Symbol," error=",GetLastError());
      return INIT_FAILED;
   }
   g_session_id=StringFormat("D025V103_%s_%I64d_%I64d",_Symbol,(long)TimeGMT(),(long)GetTickCount64());
   ...
   Print("[D025V][START] Virtual Path 1.03 symbol=",_Symbol," max_hold=",InpMaxHoldHours,
         "h | NO_ORDERS | NO_BALANCE_DEPENDENCY | LOCKED_V0_ENTRIES");
...
void OnDeinit(const int reason)
{
   ...
   Print("[D025V][STOP] reason=",reason);
}
