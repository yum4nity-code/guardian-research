...
Write-Host ""
Write-Host "=== GUARDIAN PHENOMENON DISCOVERY - PHASE B ATLAS V1.01 ===" -ForegroundColor Cyan
...
try {
    ...
    foreach ($required in @($Summary,$Shortlist,$Survivors)) {
        if (-not (Test-Path -LiteralPath $required)) { throw "Required Phase B artifact missing: $required" }
    }

    $pubArgs = @(
        "--phase","phase-b-atlas",
        "--status","PASS",
        "--summary","Phase B coarse phenomenon atlas completed. 2024 discovery and 2025 internal confirmation finished; 2026 untouched.",
        "--artifact",$Summary,
        "--artifact",$Shortlist,
        "--artifact",$Survivors
    )
    $pubCode = Invoke-PythonFile $Publisher $pubArgs
    if ($pubCode -ne 0) { throw "Atlas completed but automatic GitHub publication failed, exit=$pubCode" }

    Write-Host ""
    Write-Host "=== PHASE B ATLAS READY AND PUBLISHED ===" -ForegroundColor Green
    exit 0
}
catch {
