#!/usr/bin/env python3
"""Build causal derivative-context matrices for Phase E-A v1.01.

Fixes v1.00 CSV timestamp typing: timestamp fields are read as strings by
csv.DictReader, so every lag-time arithmetic path now casts explicitly to int.
...
def load_funding(path: Path) -> tuple[list[int], list[float]]:
    ...
    if ts != sorted(ts):
        raise RuntimeError(f"funding timestamps are not sorted: {path}")
    return ts, vals
...
def lag(rows: list[dict], idx: int, field: str, bars: int):
    ...
    if j < 0:
        return None
    ts_now = int(rows[idx]["timestamp_ms"])
    ts_old = int(rows[j]["timestamp_ms"])
    if ts_now - ts_old != bars * INTERVAL_MS:
        return None
...
def main() -> int:
    ...
    manifest = {"schema": 1, "builder_version": "1.01", "symbols": {}}
    ...
    for symbol in ("BTCUSDT", "ETHUSDT"):
        ...
        manifest["symbols"][symbol] = {
            ...
            "funding_first_timestamp_ms": fund_ts[0] if fund_ts else None,
            "funding_last_timestamp_ms": fund_ts[-1] if fund_ts else None,
            "funding_rows": len(fund_ts),
            "output_file": str(out_path),
        }
        print(f"{symbol}: rows={len(base_rows)} missing_context={missing_context} missing_funding={missing_funding} funding_rows={len(fund_ts)}")
