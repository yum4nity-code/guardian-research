...
ENGINE_VERSION="V111-EURUSD-H11-120M-OOS-FTMO-V1.0"
...
def provenance(root):
    ...
    d=pd.read_csv(p)
    m=(d["target_market"].astype(str)==MARKET)&(d["cell_type"].astype(str)=="hour")&(d["horizon_min"].astype(int)==HORIZON)&(d["orientation"].astype(int)==ORIENTATION)
    q=d[m].copy()
    q=q[q["cell_id"].astype(str).str.contains("11",regex=False)]
    q=q[q["n"].astype(int)==EXPECTED_PREOOS_N]
    if len(q)!=1:
        raise RuntimeError(f"Expected one exact EURUSD H11 120m SHORT V111 row, got {len(q)}: {q.to_dict(orient='records')}")
    r=q.iloc[0]
    if float(r["mean_bp"])<=0:raise RuntimeError("Pre-OOS provenance mean is not positive")
    return p,r.to_dict()
...
def source_stage(root,out):
    ...
    q025,q10,q50,q90,q975=month_bootstrap(ct,a)
    lm,lj=loo(ct,a);m=mean_bp(a);cm=mean_bp(b)
    existence="NEGATIVE" if m<=0 else ("POSITIVE_CONFIRMED" if np.isfinite(q10) and q10>0 else "POSITIVE_UNCERTAIN")
    ledger=pd.DataFrame({
        "source_entry_label":ct,"source_exit_label":ct+pd.Timedelta(minutes=HORIZON),
        "source_entry_px":ce,"source_exit_px":cx,"source_return":a,"source_bp":a*1e4,
        "orientation":ORIENTATION,"direction":"SHORT"
    })
    ledger.to_csv(out/"EURUSD_SOURCE_OOS_EVENT_LEDGER.csv",index=False)
    res={
        "source_v111_run":SOURCE_RUN,"candidate_id":str(prov["candidate_id"]),"cell_id":str(prov["cell_id"]),
        "window":{"start":str(START),"end_exclusive":str(END)},"n":int(len(a)),"control_n":int(len(b)),
        "mean_bp":m,"control_mean_bp":cm,"effect_bp":m-cm,
        "median_bp":float(np.median(a)*1e4) if len(a) else np.nan,"win_rate":float((a>0).mean()) if len(a) else np.nan,
    ...
    jd(out/"SOURCE_OOS_SUMMARY.json",res)
    return res,ledger
...
def align(E,sym):
    ...
    if elig.empty:raise RuntimeError("No clock offset has >=90% coverage")
...
def execute(E,sym,off):
    ...
    for _,r in E.iterrows():
        eb=pd.Timestamp(r["source_entry_label"])+pd.Timedelta(hours=off)
        xb=pd.Timestamp(r["source_exit_label"])+pd.Timedelta(hours=off)
        ...
        gross=-(xbi/ebi-1)*1e4;net=(ebi-xask)/ebi*1e4
        base.update({"execution_available":True,"entry_time":et,"exit_time":xt,"entry_bid":ebi,"entry_ask":eask,
                     "exit_bid":xbi,"exit_ask":xask,"entry_spread_bp":(eask-ebi)/ebi*1e4,
                     "exit_spread_bp":(xask-xbi)/xbi*1e4,"ftmo_bid_gross_bp":gross,"ftmo_exec_bp":net,
...
def main():
    ...
    src,E=source_stage(root,out)
    print("SOURCE OOS:",json.dumps(src,indent=2),flush=True)
    if float(src["mean_bp"])<=0:
        ...
        print("STOP: source fresh mean <= 0; FTMO stage not run.",flush=True);return
    ...
    try:
        ...
        sym=resolve(MARKET);print("FTMO SYMBOL:",sym,flush=True)
        D,S,best=align(E,sym);D.to_csv(out/"EURUSD_ALIGNMENT_EVENT_MATRIX.csv",index=False);S.to_csv(out/"EURUSD_ALIGNMENT_SCAN.csv",index=False)
        print("BEST ALIGNMENT:",best,flush=True)
        X=execute(E,sym,int(best["offset_hours"]));X.to_csv(out/"EURUSD_FTMO_EXECUTION_LEDGER.csv",index=False)
        ex=exec_summary(X)
        grid=[]
        vals=X.loc[X["execution_available"]==True,"ftmo_exec_bp"].to_numpy(float)
        for c in [0,.25,.5,.75,1,1.5,2,2.5,3,4,5]:
            y=vals-c;grid.append({"extra_cost_bp":c,"n":int(len(y)),"mean_net_bp":float(y.mean()),"win_rate":float((y>0).mean())})
        pd.DataFrame(grid).to_csv(out/"EURUSD_EXTRA_COST_GRID.csv",index=False)
        final={"source_oos":src,"best_alignment":best,"execution":ex,"ftmo_identity":ident,"2026_accessed":False}
        jd(out/"FINAL_SUMMARY.json",final)
        (out/"SUCCESS.flag").write_text("OK\n2026_ACCESS=false\n",encoding="utf-8")
        print("EXECUTION:",json.dumps(ex,indent=2),flush=True)
        print("=== EURUSD V111 H11 120M PIPELINE COMPLETE ===",flush=True)
