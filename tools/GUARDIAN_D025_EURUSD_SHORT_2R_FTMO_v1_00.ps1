...
$Analyzer = Join-Path $Repo "tools\analyze_d025_eurusd_short_2r_ftmo_v1_00.py"
$Stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$RunId = "D025_EURUSD_SHORT_2R_FTMO_$Stamp"
...
$Zip = Join-Path ([Environment]::GetFolderPath("Desktop")) "GUARDIAN_D025_EURUSD_SHORT_2R_FTMO_$Stamp.zip"
...
exact=[n for n in names if n.upper()=="EURUSD"]
...
$Portable = "D:\MT5_Backtests\Terminals\FTMO_D025_EURUSD_$Stamp"
...
$EaBase="D025_EURUSD_SHORT_2R_FTMO_$Stamp"
...
while(-not (Test-Path $Ex5)){
    ...
    if($ct -match '(?i)Result:\s*[1-9][0-9]*\s+errors?' -or $ct -match '(?i)error\s+[0-9]+:'){throw "Compile failed. "+$ct}
...
while(-not (Test-Path (Join-Path $CommonOut "COMPLETE.txt"))){
    ...
    if(((Get-Date)-$last).TotalSeconds -ge 30){
        Write-Host "D025 EURUSD backtest running..."
        $last=Get-Date
    }
...
Step "8/9 Analyze frozen EURUSD SHORT +2R"
