...
VERSION="EA01-XR-RSI-LONG-V1-FTMO-TRANSPORT-1.1-DST"
...
FTMO_TZ=ZoneInfo("Europe/Athens")
...
def ftmo_offset_hours(t):
    p=pd.Timestamp(t)
    if p.tzinfo is None:p=p.tz_localize("UTC")
    else:p=p.tz_convert("UTC")
    return int(p.to_pydatetime().astimezone(FTMO_TZ).utcoffset().total_seconds()//3600)
...
def main():
    ...
    try:
        ...
        for _,r in no.iterrows():
            ...
            eo=ftmo_offset_hours(e);xo=ftmo_offset_hours(x)
            rec["entry_ftmo_offset_hours"]=eo
            rec["exit_ftmo_offset_hours"]=xo
            rec["e_dst"]=m1_open(q,e+pd.Timedelta(hours=eo))
            rec["x_dst"]=m1_close_before(q,x+pd.Timedelta(hours=xo))
            rows.append(rec)
        A=pd.DataFrame(rows)
        stats=[]
        for off in OFFSETS:
        ...
        if elig.empty:raise RuntimeError("No fixed-offset diagnostic >=90% coverage")
        best_fixed=elig.sort_values(["median_abs_two_leg_price_diff_bp","offset_hours"]).iloc[0].to_dict()

        ep=pd.to_numeric(A["e_dst"],errors="coerce");xp=pd.to_numeric(A["x_dst"],errors="coerce")
        ok=ep.notna()&xp.notna()
        if float(ok.mean())<.90:raise RuntimeError("DST alignment coverage below 90%")
        se=A.loc[ok,"source_entry"].to_numpy(float);sx=A.loc[ok,"source_exit"].to_numpy(float)
        fe=ep[ok].to_numpy(float);fx=xp[ok].to_numpy(float)
        de=np.abs((fe/se-1)*1e4);dx=np.abs((fx/sx-1)*1e4)
        srcv=A.loc[ok,"source_r"].to_numpy(float);riskv=A.loc[ok,"source_risk"].to_numpy(float)
        fr=(fx-fe)/riskv
        dst_alignment={"mapping":"EUROPE_ATHENS_GMT2_GMT3_DETERMINISTIC","n":int(ok.sum()),
          "coverage":float(ok.mean()),"median_abs_entry_price_diff_bp":float(np.median(de)),
          "median_abs_exit_price_diff_bp":float(np.median(dx)),
          "median_abs_two_leg_price_diff_bp":float(np.median(np.r_[de,dx])),
          "source_vs_ftmo_r_corr":float(np.corrcoef(srcv,fr)[0,1]) if len(srcv)>2 and np.std(srcv)>0 and np.std(fr)>0 else np.nan,
          "entry_offset_counts":{str(int(k)):int(v) for k,v in A.entry_ftmo_offset_hours.value_counts().sort_index().items()},
          "exit_offset_counts":{str(int(k)):int(v) for k,v in A.exit_ftmo_offset_hours.value_counts().sort_index().items()}}
        pd.DataFrame([dst_alignment]).to_csv(out/"EA01_DST_ALIGNMENT.csv",index=False)

        exrows=[]
        for _,r in no.iterrows():
            eoff=ftmo_offset_hours(r.entry_time_utc);xoff=ftmo_offset_hours(r.source_exit_time_utc)
            eb=r.entry_time_utc+pd.Timedelta(hours=eoff);xb=r.source_exit_time_utc+pd.Timedelta(hours=xoff)
            ...
            rec={"source_entry_time_utc":r.entry_time_utc,"source_exit_time_utc":r.source_exit_time_utc,
                 "mapped_entry_utc":eb,"mapped_exit_utc":xb,"entry_ftmo_offset_hours":eoff,"exit_ftmo_offset_hours":xoff,"source_entry":float(r.entry),
        ...
        result={
          ...
          "non_overlap_n":int(len(no)),"alignment":dst_alignment,"fixed_alignment_diagnostic":best_fixed,"symbol":sym,"ftmo_identity":ident,
