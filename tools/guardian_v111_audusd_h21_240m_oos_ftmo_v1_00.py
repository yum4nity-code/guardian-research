...
def provenance(root):
    ...
    q=d[m].copy()
    q=q[q["cell_id"].astype(str).str.contains("21",regex=False)]
    if len(q)!=1:raise RuntimeError(f"Expected one exact AUDUSD H21 240m SHORT row, got {len(q)}: {q.to_dict(orient='records')}")
...
def source_stage(root,out):
    ...
    label="NEGATIVE" if m<=0 else ("POSITIVE_CONFIRMED_ENDPOINT" if np.isfinite(q10) and q10>0 else "POSITIVE_UNCERTAIN_ENDPOINT")
    E=pd.DataFrame({"source_entry_label":ct,"source_exit_label":ct+pd.Timedelta(minutes=HORIZON),"source_entry_px":ce,"source_exit_px":cx,
                    "source_return":a,"source_bp":a*1e4,"orientation":ORIENTATION,"direction":"SHORT"})
    E.to_csv(out/"AUDUSD_SOURCE_OOS_EVENT_LEDGER.csv",index=False)
    res={"candidate_id":str(prov["candidate_id"]),"cell_id":str(prov["cell_id"]),"n":int(len(a)),"control_n":int(len(b)),
         ...
         "v2_endpoint_label":label,
         "scientific_caveat":"H21 family 120m endpoint already opened in V112; this is 240m endpoint-extension evidence, not independent family confirmation.",
         "2026_accessed":False}
...
def execute(E,sym,off):
    ...
    for _,r in E.iterrows():
        ...
        base.update({"execution_available":True,"entry_time":et,"exit_time":xt,"entry_bid":ebi,"entry_ask":eask,"exit_bid":xbi,"exit_ask":xask,
                     "entry_spread_bp":(eask-ebi)/ebi*1e4,"exit_spread_bp":(xask-xbi)/xbi*1e4,"ftmo_bid_gross_bp":gross,"ftmo_exec_bp":net,
...
def main():
    ...
    try:
        ...
        D.to_csv(out/"AUDUSD_ALIGNMENT_EVENT_MATRIX.csv",index=False);S.to_csv(out/"AUDUSD_ALIGNMENT_SCAN.csv",index=False)
        print("BEST ALIGNMENT:",best,flush=True)
        X=execute(E,sym,int(best["offset_hours"]));X.to_csv(out/"AUDUSD_FTMO_EXECUTION_LEDGER.csv",index=False)
        ex=exec_summary(X)
        vals=X.loc[X["execution_available"]==True,"ftmo_exec_bp"].to_numpy(float)
        pd.DataFrame([{"extra_cost_bp":c,"n":int(len(vals)),"mean_net_bp":float((vals-c).mean()),"win_rate":float(((vals-c)>0).mean())} for c in [0,.25,.5,.75,1,1.5,2,2.5,3,4,5]]).to_csv(out/"AUDUSD_EXTRA_COST_GRID.csv",index=False)
        ...
        print("=== AUDUSD V111 H21 240M ENDPOINT PIPELINE COMPLETE ===",flush=True)
