...
def availability_probe(symbol):
    probes=[]
    windows=[
        ("2023", pd.Timestamp("2023-06-15 12:00"), pd.Timestamp("2023-06-15 13:00")),
        ("2024", pd.Timestamp("2024-06-13 12:00"), pd.Timestamp("2024-06-13 13:00")),
        ("2025", pd.Timestamp("2025-06-12 12:00"), pd.Timestamp("2025-06-12 13:00")),
    ]
    for label,a,b in windows:
        arr=mt5.copy_rates_range(symbol,mt5.TIMEFRAME_M1,mt5_utc_dt(a),mt5_utc_dt(b))
        n=0 if arr is None else len(arr)
        probes.append({"label":label,"m1_bars":int(n),"last_error":mt5.last_error()})
    return probes
...
def parity(name,E,n_exp,mean_exp):
    ...
    out["pass"]=bool(out["n_match"] and out["mean_abs_diff_bp"]<=1e-4)
    return out
...
def ticks(symbol,start,end):
    ...
    a=mt5.copy_ticks_range(symbol,mt5_utc_dt(start),mt5_utc_dt(end),mt5.COPY_TICKS_ALL)
...
def rates_m1(symbol,start,end):
    ...
    a=mt5.copy_rates_range(symbol,mt5.TIMEFRAME_M1,mt5_utc_dt(start),mt5_utc_dt(end))
    if a is None or len(a)==0:return pd.DataFrame()
...
def audit_execution(E,symbol,direction,mode):
    ...
    for i,r in E.iterrows():
        ...
        base={
            **r.to_dict(),
            "entry_available":en is not None,
            "exit_available":ex is not None,
            "execution_available":en is not None and ex is not None,
            "missing_leg":("both" if en is None and ex is None else "entry" if en is None else "exit" if ex is None else ""),
            "ftmo_entry_time":pd.NaT,"ftmo_exit_time":pd.NaT,
            ...
            "ftmo_bid_return":np.nan,"ftmo_exec_return":np.nan,"ftmo_exec_bp":np.nan,
            "source_bp":float(r.source_return)*1e4,
            "entry_source_vs_ftmo_bid_bp":np.nan,"exit_source_vs_ftmo_bid_bp":np.nan,
            "hold_hours":np.nan,
        }
...
def stats(X):
    if "execution_available" not in X.columns:
        return {"source_events":int(len(X)),"execution_events":0,"coverage":0.0}
    z=X[X.execution_available==True].copy()
    if "ftmo_exec_return" not in z.columns:
        return {"source_events":int(len(X)),"execution_events":0,"coverage":0.0}
...
def cost_grid(X,label):
    rows=[]
    if "execution_available" not in X.columns or "ftmo_exec_bp" not in X.columns:
        return pd.DataFrame(rows,columns=["strategy","extra_cost_bp_after_observed_spread","n","mean_net_bp","win_rate","max_drawdown_cumsum_bp"])
    z=X[X.execution_available==True].copy()
...
def main():
    ...
    try:
        ...
        probes={"USDCHF":availability_probe(chf),"AUDUSD":availability_probe(aud)}
        print("FTMO M1 AVAILABILITY PROBE:",probes,flush=True)
        ...
        diag={
            "m1_availability_probe":probes,
            "V69":{"source_events":len(x69),"entry_available":int(x69["entry_available"].sum()),"exit_available":int(x69["exit_available"].sum()),"execution_available":int(x69["execution_available"].sum()),"missing_leg_counts":x69["missing_leg"].value_counts(dropna=False).to_dict()},
            "V112":{"source_events":len(x112),"entry_available":int(x112["entry_available"].sum()),"exit_available":int(x112["exit_available"].sum()),"execution_available":int(x112["execution_available"].sum()),"missing_leg_counts":x112["missing_leg"].value_counts(dropna=False).to_dict()},
            "2026_accessed":False
        }
        write_json(out/"FTMO_AVAILABILITY_DIAGNOSTIC.json",diag)
        print("FTMO EXECUTION AVAILABILITY:",diag,flush=True)
        ...
        result={
            "ftmo_availability_diagnostic":diag,
        ...
        if int(x69["execution_available"].sum())==0 or int(x112["execution_available"].sum())==0:
            (out/"EXECUTION_DATA_INCOMPLETE.flag").write_text("SOURCE_PARITY=PASS\nFTMO_EXECUTION_DATA_INCOMPLETE=true\n2026_ACCESS=false\n",encoding="utf-8")
        else:
